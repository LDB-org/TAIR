...
def test_named_frame_preserves_raw_command_and_cannot_dispatch_embedded_frame():
    tools = [tool('bash', {'command': {'type': 'string'}, 'timeout': {'type': 'number'}}, ['command']),
             tool('ls', {'path': {'type': 'string'}}, [])]
    p = Protocol(tools, raw_required_tail=True, named=True)
    command = 'echo "hello"\nls\n[null]'
    call = {'name': 'bash', 'arguments': {'command': command, 'timeout': 3}}
    assert p.encode(call) == 'bash\n[3]\n' + command
    assert p.decode(p.encode(call), 'stop') == call
    assert p.decode('ls\n[null]', 'stop') == {'name': 'ls', 'arguments': {}}
    for raw in ['A\n[null]', 'unknown\n[]', 'ls\n[null]\nbash\n[null]\necho hi']:
        with pytest.raises(ValueError):
            p.decode(raw, 'stop')


def test_named_identifiers_are_literal_and_invalid_names_fail_closed():
    p = Protocol([tool('files.read', {'path': {'type': 'string'}}, ['path'])], named=True)
    assert p.decode('files.read\n[]\nx', 'stop')['arguments'] == {'path': 'x'}
    with pytest.raises(ValueError):
        p.decode('filesXread\n[]\nx', 'stop')
    with pytest.raises(ValueError):
        Protocol([tool('bad\nname', {}, [])], named=True)
...
def test_named_bounded_command_has_a_terminal_frame_boundary():
    bash = tool('bash', {'command': {'type': 'string'}, 'timeout': {'type': 'number'}}, ['command'])
    p = Protocol([bash], named=True)
    frame = 'bash\n["python3 -m unittest",30]'
    assert p.decode(frame, 'stop')['arguments']['command'] == 'python3 -m unittest'
    for extra in ['\npython3 -m unittest', '\nbash\n["echo hi",30]']:
        with pytest.raises(ValueError):
            p.decode(frame + extra, 'stop')


@pytest.mark.parametrize('mode,frame', [
    ('engine_raw', 'A\n[3]\necho ok'),
    ('engine_named', 'bash\n[3]\necho ok'),
    ('engine_named_bounded', 'bash\n["echo ok",3]'),
])
def test_inference_selects_requested_engine_layout(monkeypatch, mode, frame):
    sent = []
    def response(req, timeout):
        sent.append(json.loads(req.data))
        return io.StringIO(json.dumps({'choices': [{'finish_reason': 'stop', 'message': {'content': frame}}]}))
    monkeypatch.setattr(module.urllib.request, 'urlopen', response)
    result = module.infer({'protocol_mode': mode, 'session_id': 'test',
                           'tools': [tool('bash', {'command': {'type': 'string'}, 'timeout': {'type': 'number'}}, ['command'])],
                           'messages': [{'role': 'user', 'content': 'Execute echo ok'}]}, 'http://localhost')
    assert result['call'] == {'name': 'bash', 'arguments': {'command': 'echo ok', 'timeout': 3}}
    assert 'structured_outputs' in sent[0] and 'tools' not in sent[0]
