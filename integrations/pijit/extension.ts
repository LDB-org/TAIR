...
export default function(pi: any) {
  let cwd = process.cwd();
  let ui: any;
  let totals = { generated: 0, controls: 0, hits: 0, edits: 0 };
  function record(result: any, edit = false) {
    ...
    ui?.setStatus('pijit', `pijit · gen ${totals.generated} · cls ${totals.controls} · JIT ${totals.hits}/${totals.edits}`);
  }
  ...
  pi.registerProvider('pijit', {
    ...
    streamSimple(model: any, context: any, options: any) {
      ...
      (async () => {
        ...
        try {
          const result = await bridge({ action: 'chat', context, cwd }, options?.signal);
          record(result);
          ...
          if (call.name === 'reply_user') {
            ...
            message.content = [{ type: 'toolCall', id, name: call.name, arguments: call.arguments }];
  ...
  pi.registerTool({
    ...
    description: 'Edit an EXISTING Python file with compact source-bound operations and a persistent JIT codebook. '
      + 'Supports call keyword/default changes, option aliases, entry guards and catch/return. '
      + 'Read first. Provide a precise task, not old/new source. Other languages and unsupported changes use edit/write. '
    ...
    async execute(_id: string, args: any, signal: AbortSignal, onUpdate: any, ctx: any) {
      ...
      const result = await bridge({ action: 'edit', cwd: ctx.cwd, ...args }, signal);
      record(result, true);
      const summary = `Applied compact edit to ${result.path}. JIT hit: ${result.cache_hit}. `
        + `Generated argument tokens: ${result.generated_argument_tokens}; controls: ${result.classification_control_records}.\n`
  ...
  pi.registerCommand('jit', {
    ...
    handler: async (_args: string, ctx: any) => {
      ...
      ctx.ui.notify(`pijit: ${count} source-bound entries; this session ${totals.hits}/${totals.edits} edit hits.\n`
        ...
        + `State: ${directory}\nCompact edits: Python only. Reuse: same wording (one integer may vary), exact source version.\n`
