...
def server_plan_budget():
    ...
    try:
        ...
        if error.code == 404:
            trace_update(prefix_cache_supported=False)
            return False
        raise
...
def generic_plan_chat(payload):
    ...
    candidates = book.candidates(task, {}, limit=candidate_limit) if any(t['name']=='write' for t in tools) else []
    template_book = None
    if reuse_enabled and os.environ.get('PIJIT_PLAN_TEMPLATES') == '1':
    ...
    if success_reply:
        policy += ('\nAn action plan may append a final step named on_success_reply with arguments {content: a short final reply}, to deliver ONLY '
                   'after every operation succeeds. Include it only if this plan finishes ALL requested work '
                   'and no unseen results need interpretation. Omit it for investigation, read/search, '
    ...
    success_text = tool_plan.split_success_reply(response['call']['arguments'])[1] if success_reply and call['name']=='plan' else None
    trace_update(planned_success_reply=bool(success_text))
    return dict(response, call=call, plan_task=task, on_success_reply=success_text, reused_content_ids=reused_contents,
