...
def branches(tools, candidates, success_reply=False):
    ...
    if success_reply:
        for option in options:
            ...
            sequence['items']['anyOf'].append(obj(dict(name=dict(const='on_success_reply'),
                arguments=obj(dict(content=dict(type='string', maxLength=600))))))
    return options
...
def split_success_reply(arguments):
    ...
    text = steps.pop()['arguments']['content']
...
def continuation(index, tools, candidates, option, success_reply=False):
    ...
    if success_reply:
        instruction += ('When THIS plan contains all remaining work and checks, append '
                        '{"name":"on_success_reply","arguments":{"content":"your concise final answer"}} '
                        'as the LAST item in rest or steps. Prefer this for a complete write/edit plus check plan '
                        'instead of spending another model turn to announce success. This is a deferred reply, '
                        'not an early success claim: the executor withholds it until every operation succeeds. '
                        'If any result must be read or interpreted before finishing, omit that item. '
