...
import preset_edits
from structural_edit_protocol import preserves_argument_destination
...
def alias_binding(source, task, *, existing_alias=False):
    ...
    if len(calls) != 1:
        return None
    if not preserves_argument_destination(calls[0], match['alias']):
        return None
    # Reuse requires a recognized argparse receiver, not merely a method with this name.
    recognized = False
    for keyword in calls[0].keywords:
        if keyword.arg not in ('default', 'help', 'required'):
            continue
        try:
            value = ast.literal_eval(keyword.value)
            preset_edits.set_cli_keyword(source, match['option'], keyword.arg, value, value)
            recognized = True
            break
