...
class Protocol:
    def __init__(self, tools, raw_required_tail=False, named=False):
        ...
        for i, tool in enumerate(tools):
            ...
            pattern(schema)
            props = fields(schema)
            required_props = [(k, s) for k, s in props if k in schema.get('required', [])]
            candidate = required_props[-1] if required_props else None
            tail = candidate if raw_required_tail and candidate and candidate[0] == 'command' else (props[-1] if props else None)
            raw_key = tail[0] if tail and tail[1]['type'] == 'string' and tail[0] in schema.get('required', []) else None
            prefix = dict(schema)
            if raw_key:
                prefix['properties'] = {k: s for k, s in props if k != raw_key}
                prefix['required'] = [k for k in schema.get('required', []) if k != raw_key]
            code = tool['name'] if named else chr(65 + i)
            if named and re.fullmatch(r'[A-Za-z_][A-Za-z0-9_.-]*', code) is None:
                raise ValueError('Invalid tool name for named frame')
            self.entries[code] = (tool, prefix, raw_key)
            rules.append(re.escape(code) + r'\n' + pattern(prefix) + (r'\n[\s\S]*' if raw_key else ''))
        ...
        self.instruction = ('You are a coding agent. Every response MUST be exactly one tool frame below. '
                            f'Select the appropriate {"tool name" if named else "letter"} FIRST, then generate its values. Never output prose outside a tool. '
    ...
    def decode(self, raw, finish_reason):
        ...
        tool, prefix, raw_key = self.entries[code]
...
def infer(request, upstream):
    if request.get('protocol_mode', 'engine') == 'native':
        return infer_native(request, upstream)
    if request.get('protocol_mode', 'engine') not in ('engine', 'engine_raw', 'engine_named', 'engine_named_bounded'):
        raise ValueError('Unsupported protocol mode')
    protocol = Protocol(request['tools'], raw_required_tail=request.get('protocol_mode') in ('engine_raw', 'engine_named'),
                        named=request.get('protocol_mode') in ('engine_named', 'engine_named_bounded'))
...
def infer_native(request, upstream):
    """Native vLLM tools baseline with the same non-streaming SSH transport."""
    messages = [{'role': 'system', 'content': request.get('systemPrompt', '')}]
    for msg in request['messages']:
        if msg['role'] == 'assistant':
            messages.append({'role': 'assistant', 'content': ''.join(c['text'] for c in msg['content'] if c['type'] == 'text') or None, 'tool_calls': [
                {'id': c['id'], 'type': 'function', 'function': {'name': c['name'],
                 'arguments': json.dumps(c['arguments'], ensure_ascii=False)}}
                for c in msg['content'] if c['type'] == 'toolCall']})
        elif msg['role'] == 'toolResult':
            messages.append({'role': 'tool', 'tool_call_id': msg['toolCallId'],
    ...
    payload = {'model': '/model', 'messages': messages, 'temperature': 0, 'max_tokens': 4096,
               ...
               'tool_choice': 'auto', 'parallel_tool_calls': False, 'return_token_ids': True,
               'cache_salt': request['session_id'],
               'chat_template_kwargs': {'thinking': False, 'enable_thinking': False}}
    started = time.perf_counter()
    req = urllib.request.Request(upstream + '/v1/chat/completions', data=json.dumps(payload).encode(), headers={'Content-Type': 'application/json'})
    ...
    trace = {'seconds': time.perf_counter() - started, 'message': message, 'finish_reason': choice['finish_reason'],
             'token_ids': choice.get('token_ids'), 'usage': data.get('usage'), 'metrics': data.get('metrics'),
             'request_id': data.get('id'), 'protocol_mode': 'native'}
    if choice['finish_reason'] not in ('stop', 'tool_calls') or not message.get('tool_calls'):
        return {'call': None, 'error': 'Native response incomplete or has no tool call', 'trace': trace}
    try:
        calls = [{'name': c['function']['name'], 'arguments': json.loads(c['function']['arguments'])}
                 for c in message['tool_calls']]
    except (ValueError, KeyError, TypeError) as exc:
        return {'call': None, 'error': str(exc), 'trace': trace}
    return {'call': calls[0], 'calls': calls, 'native_text': message.get('content') or '', 'trace': trace}
