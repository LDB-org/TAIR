...
def main(args):
    ...
    (out / 'manifest.json').write_text(json.dumps({
        'arms': ['native', 'generation', 'accelerated'], 'rounds': 2, 'repeats': args.repeats,
    ...
    with tempfile.TemporaryDirectory(prefix='tair-agent-jit-') as temp:
        for repeat in range(args.repeats):
            for name, scenario in SCENARIOS.items():
                for round_ in (1, 2):
                    arms = ['native', 'generation', 'accelerated']; rng.shuffle(arms)
                    for arm in arms:
                        ...
                        enabled = str(int(arm == 'accelerated'))
                        os.environ.update(PIJIT_SCHEMA_ACTIONS=enabled, PIJIT_JIT_ACTIONS=enabled)
                        row = b.attempt(project, state, folder, 'c4' if arm == 'accelerated' else arm,
