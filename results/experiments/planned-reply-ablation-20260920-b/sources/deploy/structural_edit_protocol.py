...
def preserves_argument_destination(node, alias):
    """Conservatively guard argparse-like calls against prepend changing implicit dest."""
    if not isinstance(node.func, ast.Attribute) or node.func.attr != 'add_argument':
        return True
    destination_arg = next((k.value for k in node.keywords if k.arg == 'dest'), None)
    if destination_arg is not None:
        if not isinstance(destination_arg, ast.Constant):
            return False
        if isinstance(destination_arg.value, str):
            return True
        if destination_arg.value is not None:
            return False
    options = [a.value for a in node.args if isinstance(a, ast.Constant) and isinstance(a.value, str)]
    if len(options) != len(node.args) or not options or not isinstance(alias, str):
        return False
    if not all(s.startswith('-') for s in [alias, *options]):
        return False
    def destination(values):
        return next((s for s in values if s.startswith('--')), values[0]).lstrip('-').replace('-', '_')
    return destination(options) == destination([alias, *options])
...
def patch(source,nodes,edit):
    ...
    if op in ('arg','kw') and isinstance(node,ast.Call):
        ...
        if isinstance(node.func, ast.Attribute) and node.func.attr == 'add_argument':
            try: alias = ast.literal_eval(value)
            except (ValueError, SyntaxError): alias = None
            if not preserves_argument_destination(node, alias):
                raise ValueError('Alias would change or obscure the argument destination; use a normal edit preserving dest')
        if node.args or node.keywords:
            a=min(span(source,argument)[0] for argument in node.args+node.keywords)
            return a,a,value+', '
...
def prompt(source, semantic_targets=False):
    text = ('Return only a compact JSON array of 1..8 operations. Choose operations and targets yourself; generate Python EXPRESSIONS only for values. Runtime preserves source and builds indentation/control flow. '
    ...
    if semantic_targets:
        labels=[]
        for key,node in catalogue(source).items():
            label=node.name if key.startswith('F') else ast.unparse(node.func) if key.startswith('C') else ast.get_source_segment(source,node).splitlines()[0][:70]
            labels.append(key+'='+label)
        text+='\nTARGET MEANINGS: '+ '; '.join(labels)
        text+='\nExamples on unrelated toy code: add a positional string argument using [["arg","C8_4","\'new\'"]]; catch an existing statement failing using [["catch_return","S9_4","LookupError","None"]]. Use guard_raise only to reject a condition with an exception expression, never to catch another statement. kw names must be actual keyword arguments accepted by the targeted call. An array may contain multiple operations; satisfy every requested modification.'
    return text
