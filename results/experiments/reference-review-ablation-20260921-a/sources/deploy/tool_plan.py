...
def branches(tools, candidates, success_reply=False, reuse_review=False, write_references=False):
    references = [i for i, entry in enumerate(candidates) if not entry.get("plan_template")]
    if write_references and references:
        tools = copy.deepcopy(tools)
        for tool in tools:
            if tool["name"] == "write":
                content = tool["parameters"]["properties"]["content"]
                check = dict(requirement_check=dict(type='string', maxLength=320)) if reuse_review else {}
                variants = [content, obj(dict(**check, stored=dict(type='integer', enum=references)))]
                if reuse_review:
                    variants.append(obj(dict(**check, generated=content)))
                tool["parameters"]["properties"]["content"] = dict(anyOf=variants)
    ...
    for candidate_index, entry in enumerate(candidates):
        ...
        if write_references and not entry.get('plan_template'):
            reference = dict(requirement_check='...') if reuse_review else {}
            reference['stored'] = candidate_index
            description += 'Any write may use content='+json.dumps(reference)+' to reference these exact bytes. '
        options.append(dict(name='plan', description=description +
                            ...
                            '\nOBSERVED FILE PATHS (historical context; destination may change):\n'+
                            json.dumps(observed_paths(entry), ensure_ascii=False)+
...
def decode(response, tools, candidates, options):
    ...
    for step in steps:
        ...
        if step['name'] == 'write' and isinstance(content, dict):
            if 'stored' in content:
                entry = candidates[content['stored']]
                step['arguments']['content'] = entry['source']
                reused.append(entry['id'])
            else:
                step['arguments']['content'] = content['generated']
    validate(dict(steps=steps), schema(tools))
    return dict(name='plan', arguments=dict(steps=steps)), reused



def reuse_reviews(arguments):
    reviews = []
    steps = arguments.get('steps', arguments.get('rest', []))
    for args in [arguments.get('first', {}), *[s.get('arguments', {}) for s in steps]]:
        content = args.get('content')
        if isinstance(content, dict) and 'requirement_check' in content:
            reviews.append(dict(requirement_check=content['requirement_check'],
                                decision='reuse' if 'stored' in content else 'generate', stored=content.get('stored')))
        if 'requirement_check' in args:
            reviews.append(dict(requirement_check=args['requirement_check'],
                                decision='reuse' if args.get('use_cached_content') else 'generate'))
    return reviews
