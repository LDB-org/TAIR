...
def generic_plan_chat(payload):
    ...
    options = tool_plan.branches(tools, candidates, success_reply=success_reply,
                                 reuse_review=reuse_review, write_references=write_references)
    if recovery:
    ...
    if write_references and candidates:
        reference_format = '{"requirement_check": "...", "stored": N}' if reuse_review else '{"stored": N}'
        policy += ('\nAny write, including first or later steps, may supply content='+reference_format+' instead of a string. '
        ...
        if reuse_review:
            policy += (' Before choosing stored, FIRST write requirement_check naming the current distinguishing requirement '
                       'and the actual operation in the stored code that implements it. A similar old task is not evidence. '
                       'If the required behavior or export is missing, continue the same object with generated: the full new source '
                       'instead of stored, or generate a normal content string. Do not cite an operation absent from the stored code.')
    ...
    trace_update(reuse_reviews=tool_plan.reuse_reviews(response['call']['arguments']))
