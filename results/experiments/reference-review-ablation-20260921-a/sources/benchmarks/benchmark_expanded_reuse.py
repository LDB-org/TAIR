...
def main(out, continuation=False, task_ids=None, fresh=False, local_tokenizer=None, verify_tokenizer=False, prefix_cache=False, compact_catalog=False, reuse_routing=False, ssh_compression=False, ablation=False, empty_book=False, success_reply_ablation=False, recovery_latest=False, recovery_ablation=False, capabilities_ablation=False, execution_ablation=False, reuse_review_ablation=False, write_references_ablation=False, recovery_admission_ablation=False, warm_reuse_ablation=False, repeat_count=1, reference_review_ablation=False):
    if reference_review_ablation and (warm_reuse_ablation or recovery_admission_ablation or write_references_ablation or reuse_review_ablation or execution_ablation or success_reply_ablation or recovery_ablation or capabilities_ablation or recovery_latest or reuse_routing or compact_catalog):
        raise ValueError("Reference review ablation must isolate only same-request reuse review")
    ...
    if success_reply_ablation or recovery_ablation or capabilities_ablation or execution_ablation or reuse_review_ablation or write_references_ablation or recovery_admission_ablation or warm_reuse_ablation or reference_review_ablation:
        ablation = True
    cli = shutil.which('pi')
    if not cli:
    ...
    if reference_review_ablation:
        configured_arms=['native','tair_refs','tair_review']
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='tair-expanded-project-') as temp:
            ...
            for task_id,arm in schedule:
                ...
                overrides['PIJIT_PLAN_DISABLE_REUSE']='1' if arm in ('tair_no_reuse','tair_reply','tair_recovery','tair_capcache') else '0'
                overrides['PIJIT_PLAN_SUCCESS_REPLY']='1' if arm=='tair_reply' else '0'
                references_for_arm = arm in ('tair_refs','tair_recovered') or (warm_reuse_ablation or reference_review_ablation) and arm!='native'
                admission_for_arm = arm=='tair_recovered' or (warm_reuse_ablation or reference_review_ablation) and arm!='native'
                ...
                try:
                    ...
                    if arm!='native':
                        ...
                        assert chats and all(m.get('plan_reuse_enabled')==(arm not in ('tair_no_reuse','tair_reply','tair_recovery','tair_capcache')) for m in chats), 'Runtime reuse configuration mismatch'
