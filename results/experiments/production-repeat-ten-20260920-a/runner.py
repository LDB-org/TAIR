...
run='production-repeat-ten-20260920-a';out=ROOT/'results/experiments'/run;out.mkdir(exist_ok=False)
...
try:
 for number,(kind,spec) in enumerate(jobs[:1]*10,1):
  ...
  row.update(book_entries_before=len(round_before),book_entries_after=len(inventory()),book_after=inventory());rows.append(row);(out/'report.json').write_text(json.dumps(rows,indent=2));print(json.dumps({k:v for k,v in row.items() if k!='records'}),flush=True)
finally:
