...
def generic_plan_chat(payload):
    ...
    response = infer(messages, [dict(option, parameters=tool_plan.decoder_schema(option['parameters'])) for option in options],
                     ...
                     'For pending writes, compare the candidate HISTORICAL CONTRACT previews with the CURRENT request. '
                     'The source is deliberately deferred: select a matching contract to inspect its full source in continuation, then reuse or generate. '
