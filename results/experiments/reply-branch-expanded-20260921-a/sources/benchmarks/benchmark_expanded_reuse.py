...
def main(out, continuation=False, task_ids=None, fresh=False, local_tokenizer=None, verify_tokenizer=False, prefix_cache=False, compact_catalog=False, reuse_routing=False, ssh_compression=False, ablation=False, empty_book=False, success_reply_ablation=False, recovery_latest=False, recovery_ablation=False, capabilities_ablation=False, execution_ablation=False, reuse_review_ablation=False, write_references_ablation=False, recovery_admission_ablation=False, warm_reuse_ablation=False, repeat_count=1, argv_ablation=False, parallel_capabilities_ablation=False, reply_branch_ablation=False):
    ...
    if success_reply_ablation or recovery_ablation or capabilities_ablation or execution_ablation or reuse_review_ablation or write_references_ablation or recovery_admission_ablation or warm_reuse_ablation or argv_ablation or parallel_capabilities_ablation or reply_branch_ablation:
        ablation = True
    cli = shutil.which('pi')
    if not cli:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='tair-expanded-project-') as temp:
            ...
            for task_id,arm in schedule:
                ...
                overrides['PIJIT_PLAN_DISABLE_REUSE']='1' if arm in ('tair_no_reuse','tair_reply','tair_recovery','tair_capcache','tair_argv','tair_parallel_caps','tair_reply_branch') else '0'
                ...
                overrides['PIJIT_REPLY_BRANCH']='1' if arm=='tair_reply_branch' else '0'
                ...
                try:
                    ...
                    if arm!='native':
                        ...
                        assert chats and all(m.get('plan_reuse_enabled')==(arm not in ('tair_no_reuse','tair_reply','tair_recovery','tair_capcache','tair_argv','tair_parallel_caps','tair_reply_branch')) for m in chats), 'Runtime reuse configuration mismatch'
                        ...
                        assert all(m.get('reply_branch_enabled')==(arm=='tair_reply_branch' and not m.get('recovery_planning')) for m in chats), 'Runtime reply branch mismatch'
