...
def scoped_selectors(source,allowed=None):
    return {kind:{name:key for name,key in entries.items() if allowed is None or key in allowed}
            for kind,entries in selectors(source).items()}


def task_scope(source,task):
    """Lexical source retrieval, not an oracle: fallback to all when ambiguous."""
    nodes=base.catalogue(source)
    words=set(re.findall(r'[\w.]+',task.casefold()))
    functions={k:n for k,n in nodes.items() if k.startswith('F')}
    mentioned={k:n for k,n in functions.items() if n.name.casefold() in words}
    def inside(node,owner):
        return (owner.lineno,owner.col_offset)<=(node.lineno,node.col_offset) and (node.end_lineno,node.end_col_offset)<=(owner.end_lineno,owner.end_col_offset)
    candidates={k:n for k,n in nodes.items() if not mentioned or any(inside(n,f) for f in mentioned.values())}
    calls={}
    for key,node in candidates.items():
        if not key.startswith('C'):continue
        callee=ast.unparse(node.func).casefold()
        literal=node.args[0].value if node.args and isinstance(node.args[0],ast.Constant) else None
        literal_match=isinstance(literal,str) and len(literal)>1 and literal.casefold() in task.casefold()
        if callee in words or ('.' in callee and callee.split('.')[0] in words) or literal_match:calls[key]=node
    if not calls:return set(candidates)
    allowed=set(calls)|set(mentioned)
    for call in calls.values():
        for kind in ['F','S']:
            parents=[(k,n) for k,n in nodes.items() if k.startswith(kind) and inside(call,n)]
            if parents:allowed.add(min(parents,key=lambda item:(item[1].end_lineno-item[1].lineno,-item[1].col_offset))[0])
    return allowed


def typed_grammar(source,allowed=None):
    mapping=scoped_selectors(source,allowed);nodes=base.catalogue(source);choices=[]
...
def typed_decode(source,expected_hash,raw,finish,allowed=None):
    if base.digest(source)!=expected_hash:raise ValueError('Stale source version')
    if finish!='stop':raise ValueError('Incomplete output')
    # Revalidate grammar before translating types, even when not using constrained decoding.
    if not isinstance(raw,str) or re.fullmatch(typed_grammar(source,allowed),raw) is None:raise ValueError('Invalid typed operation')
...
def typed_prompt(source,allowed=None):
    aliases=scoped_selectors(source,allowed)
    exceptional=[name for entries in aliases.values() for name in entries if '@' in name or re.fullmatch(r'[CFS][0-9]+_[0-9]+',name)]
    targets='Other/disambiguated targets: '+','.join(exceptional)
    if allowed is not None and len(allowed)<=12:
        targets='Candidate functions: '+','.join(aliases['F'])+'; calls: '+','.join(aliases['C'])+'; statements: '+','.join(aliases['S'])
        targets+='\nUnrelated example: [["catch","resource","LookupError",null]] catches an exception from the existing resource assignment. A condition expression cannot contain an assignment or catch an exception.'
    return ('Output one JSON array containing all required edits. Engine preserves source and builds indentation. '
        ...
        'Assignment targets are the assigned variable name. '+targets)
