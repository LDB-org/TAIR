...
def attempt(folder,payload,source,regions,pristine,check,case,mode,host,line_protocol=False):
...
def main(out,host,repeats,line_protocol=False):
 ...
 manifest['previously_evaluated_cases']=list(CASES)[:6]
 manifest['new_cases']=list(CASES)[6:]
 ...
 for index,(case,rep) in enumerate(jobs):
  ...
  for arm in (['native','hybrid'] if index%2==0 else ['hybrid','native']):
   ...
   for num,mode in enumerate(modes):
    r=attempt(folder/f'attempt{num}',payloads[mode],source,regions,pristine,check,case,mode,host,line_protocol);attempts.append(r)
    if r['passed']:break
...
if __name__=='__main__':
 ...
 a=p.parse_args();main(a.out,a.host,a.repeats,a.line_protocol)
