...
class ToolContentBook(PlanBook):
    ...
    def candidates(self, task, contracts, limit=15, *, eligible=None, repair_feedback=False):
        rejected = set()
        if repair_feedback and self.path.exists():
            with self.connection() as db:
                rejected = {row[0] for row in db.execute(
                    'SELECT e.source_sha256 FROM rejections r JOIN entries e ON e.id=r.entry_id WHERE r.contract_hash=?',
                    (digest(task),))}
        return super().candidates(task, contracts, limit=limit, unique_sources=True,
            eligible=lambda entry: entry['source_sha256'] not in rejected and (eligible is None or eligible(entry)))

    def reject_content(self, source, task):
        with self.connection() as db:
            identities = [row[0] for row in db.execute('SELECT id FROM entries WHERE source_sha256=? AND source=?',
                                                      (digest(source), source))]
            db.executemany('INSERT OR IGNORE INTO rejections(entry_id,contract_hash) VALUES (?,?)',
                           [(identity,digest(task)) for identity in identities])
        return identities
...
def content_evidence(paths):
    return json.dumps(dict(version=1, validation='tool_execution_succeeded; semantic_correctness_unverified',
                           observed_paths=sorted(set(paths))), ensure_ascii=False)


def observed_paths(entry):
    try:
        evidence = json.loads(entry.get('verification', ''))
        paths = evidence['observed_paths']
        if evidence.get('version')==1 and isinstance(paths, list) and all(isinstance(p, str) for p in paths):
            return paths
...
def candidate_binding(index, entry):
    return 'Candidate '+str(index)+'; observed files: '+json.dumps(observed_paths(entry), ensure_ascii=False)


def branches(tools, candidates, success_reply=False, reuse_review=False, write_references=False, bash_argv=False, reply_branch=False, native_fallback=False):
    references = [i for i, entry in enumerate(candidates) if not entry.get("plan_template")]
    if write_references and references or bash_argv:
        tools = copy.deepcopy(tools)
        for tool in tools:
            if tool["name"] == "write" and write_references and references:
                content = tool["parameters"]["properties"]["content"]
                tool["parameters"]["properties"]["content"] = dict(anyOf=[
                    content, obj(dict(stored=dict(type="integer", enum=references)))])
            if tool["name"] == "bash" and bash_argv:
                command = tool["parameters"]["properties"]["command"]
                tool["parameters"]["properties"]["command"] = dict(anyOf=[command,
                    dict(type='array', minItems=1, maxItems=64, items=dict(type='string'))])
    if len(tools) + len(candidates) + 1 + int(reply_branch) > 16:
    ...
    for candidate_index, entry in enumerate(candidates):
        ...
        if reuse_review and not entry.get('plan_template'):
            review = dict(requirement_check=dict(type='string', maxLength=320))
            reuse['properties']['arguments'] = dict(anyOf=[
                obj(dict(**review, use_cached_content=dict(const=True), path=dict(type='string'))),
                obj(dict(**review, use_cached_content=dict(const=False), path=dict(type='string'), content=dict(type='string')))])
        ...
        if write_references and not entry.get('plan_template'):
            description += 'Any write may use content={"stored":'+str(candidate_index)+'} to reference these exact bytes. '
        options.append(dict(name='plan', description=candidate_binding(candidate_index, entry)+'\n'+description +
                            ...
                            '\nHISTORICAL USER TASK (whole request, NOT a specification of this particular file):\n'+entry['contract']+
                            '\nThis file may be a helper or test rather than the requested implementation. '
                            'Check its actual exports, behavior and dependencies against the current request. '
                            'Successful execution does not certify its purpose or correctness. '+
                            '\nCONTENT:\n'+entry['source']+
    ...
    if native_fallback:
        for option in options[:len(tools)]:
            option['parameters']['anyOf'].insert(1, copy.deepcopy(options[-1]['parameters']['anyOf'][0]))
    if reply_branch:
        options.insert(-1,dict(name='plan',description='Reply to the user: work and relevant checks are complete, or answer a conversational question, request necessary clarification, or report an unresolved blocker honestly. No tools execute in this branch.',
...
def decode(response, tools, candidates, options):
    ...
    arguments, completion = split_success_reply(arguments)
    if arguments.get('steps') == [] and completion is not None:
        if not completion['content'].strip() or completion.get('expected_outputs'):
            raise ValueError('Completion without actions requires a nonempty reply and no output expectations')
        return dict(name='reply_user', arguments=dict(content=completion['content'])), []
    reused = []
    if index == len(options)-1:
        steps = [dict(name=s['name'], arguments=copy.deepcopy(s['arguments'])) for s in arguments['steps']]
    elif index < len(tools):
        steps = copy.deepcopy(arguments['steps'] if 'steps' in arguments else
                              [dict(name=tools[index]['name'], arguments=arguments['first']), *arguments['rest']])
    else:
        ...
        for step in steps:
            ...
            if step['name'] == 'reuse_write':
                step['name'] = 'write'
                args = step['arguments']
                use_cached = args.get('use_cached_content', True)
                step['arguments'] = dict(path=args['path'], content=entry['source'] if use_cached else args['content'])
                if use_cached:
                    reused.append(entry['id'])
    ...
    for step in steps:
        ...
        if step['name']=='bash' and isinstance(command, list):
            step['arguments']['command'] = 'command -- '+shlex.join(command)
        content = step['arguments'].get('content')
        if step['name'] == 'write' and isinstance(content, dict):
            entry = candidates[content['stored']]
...
def continuation(index, tools, candidates, option, success_reply=False):
    import json
    if 'content' in option['parameters'].get('properties', {}):
        return ('Selected reply. Use the actual execution results to answer concisely. '
                'Do not claim unexecuted work or failed checks succeeded. If work remains, state that honestly. '
                'Return ONLY JSON matching this schema: '+json.dumps(option['parameters'],ensure_ascii=False))
    if index < len(tools):
        ...
        if any('steps' in variant.get('properties', {}) for variant in option['parameters'].get('anyOf', [])):
            instruction = ('Proposed first action: '+tools[index]['name']+'. This proposal may be wrong. '
                           'Determine the required first operation from the current user request before choosing the response format. '
                           'If the proposal is appropriate, put its arguments in first and other known operations in rest. '
                           'If it conflicts with the request or required order, correct it now by returning steps '
                           'with the appropriate native tools instead of first/rest. '
                           'Do not execute an unnecessary proposed action before the required first operation. ')
    elif index < len(tools)+len(candidates):
        entry = candidates[index-len(tools)]
        instruction = ('Selected '+candidate_binding(index-len(tools), entry)+'. Recheck its bytes against the current request. '
                       'If applicable, emit name=reuse_write with arguments={path: destination}; '
                       'reuse_write expands ONLY this selected candidate, not another entry in the catalog. '
                       'Do NOT regenerate matching content using write. '
        ...
        if entry.get('plan_template'):
            instruction = ('Selected plan template '+candidate_binding(index-len(tools), entry)+'. Recheck ALL current constraints. '
    ...
    if len(tools) <= index < len(tools)+len(candidates):
        variants = option['parameters']['anyOf'][0]['properties']['steps']['items']['anyOf']
        reviewed = any('anyOf' in step.get('properties', {}).get('arguments', {})
                       and step['properties'].get('name', {}).get('const') == 'reuse_write' for step in variants)
        if reviewed:
            instruction += ('For reuse_write, FIRST write requirement_check: briefly identify the current distinguishing '
                            'behavioral requirement and whether the stored bytes implement it, not merely whether the old task sounds similar. '
                            'Then set use_cached_content=true ONLY if all requirements are met. '
                            'If any requirement conflicts or is unsupported, set use_cached_content=false and provide corrected full content '
                            'in the same arguments, or use ordinary tool steps. A changed destination alone is not a conflict. ')
