...
def main(args):
    ...
    (out/'manifest.json').write_text(json.dumps({'arms': arms, 'rounds': 2, 'repeats': args.repeats,
        'scenarios': SCENARIOS, 'suite': getattr(args, 'suite', 'small'), 'seed': 921, 'outer_optimized': args.outer_optimized,
        'tokenizer_revision': args.tokenizer_revision,
        'batch_comparison': args.batch_comparison,
        'plan_context_comparison': args.plan_context_comparison,
        'contextual_arm': 'Batched with bounded root filename inventory, observed-file edit grammar, nonempty edits, no writes before first observation in a nonempty project, and deferred writes to unread existing files; context gathering and deferred generations included.',
    ...
    for path in ['benchmarks/benchmark_codebook_agent.py', 'benchmarks/compare_pijit_presets.py',
                 ...
                 'benchmarks/native_pi_trace.ts', 'benchmarks/expanded_agent_scenarios.py', 'benchmarks/verify_expanded_scenarios.py', 'integrations/pijit/bridge.py',
            ...
            for name, scenario in SCENARIOS.items():
                for round_ in (1, 2):
                    ...
                    for arm in order:
                        ...
                        row['tests_unchanged'] = all((project/path).is_file() and (project/path).read_text()==text for path,text in scenario['files'].items() if path.startswith('test'))
                        try:
    ...
    summary['limitations'] = 'Synthetic coding tasks, cold/warm pairs, real Pi loop. Different tool catalogs; no general throughput claim. Admission is compile-only; independent final behavioral oracle and original-test integrity determine success. Failed attempts and agent recovery are included. Optimized arm combines routing and transport optimizations; this is not a classifier-only ablation.'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--suite', choices=['small', 'expanded'], default='small')
