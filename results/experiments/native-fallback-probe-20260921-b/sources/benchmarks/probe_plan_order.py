...
def main(out,tokenizer,pi_context=False,forced_fallback=False):
    ...
    manifest=dict(forced_fallback=forced_fallback,pi_context=pi_context,fixtures=fixtures,guidance=GUIDANCE,source_sha256=sources,method=('Force the wrong native branch using a singleton allowed label, then generate once; identical empty-book context but native fallback schema and continuation differ; no tool execution or Agent speed claim.' if forced_fallback else 'One natural classification and generated plan per arm and fixture; identical empty-book context and schemas; no tool execution, Agent completion, or end-to-end speed claim.'))
