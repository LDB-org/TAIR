    ...
    for raw_part in spec.split(","):
        ...
        if "-" in part:
            ...
            if not left or not right:
                raise ValueError(f"malformed port range: {part!r}")
            try:
                start = int(left, 10) if left.isascii() and left.isdigit() else (_ for _ in ()).throw(ValueError())
                end = int(right, 10) if right.isascii() and right.isdigit() else (_ for _ in ()).throw(ValueError())
            except ValueError:
            ...
            try:
                port = int(part, 10) if part.isascii() and part.isdigit() else (_ for _ in ()).throw(ValueError())
            except ValueError:
