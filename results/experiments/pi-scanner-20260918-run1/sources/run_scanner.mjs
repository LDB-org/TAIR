...
let reply = null;
const tools = [createReadTool(work), createBashTool(work, { operations }), createEditTool(work), createWriteTool(work), {
...
writeFileSync(join(out, 'runtime.json'), JSON.stringify({ pi: pkg.name, version: pkg.version, node: process.version,
  ...
  worker: options.worker, host: options.host, model: '/model', started: new Date().toISOString(),
...
async function streamFn(_model, context, opts) {
  ...
  const result = ++turns <= 12 ? await remote({ ...context, tools: schemas, session_id: session }, opts.signal)
    : { call: null, error: 'Exceeded 12 model turns' };
  ...
  const message = { role: 'assistant', content: result.call ? [{ type: 'toolCall', id: `call_${turns}`, ...result.call }] : [],
...
function guardPath(path) {
  ...
  if (!absolute.startsWith(work + sep)) return false;
  let parent = absolute;
  while (!existsSync(parent)) parent = dirname(parent);
  const actual = realpathSync(parent);
  return actual === work || actual.startsWith(work + sep);
}
const prompt = `Create port_scanner.py and test_port_scanner.py in the workspace. Implement a usable Python 3 standard-library TCP connect port scanner.
...
const summary = { turns, seconds: (performance.now() - started) / 1000, reply,
...
if (!reply || !summary.all_assistant_output_is_toolcall) process.exitCode = 1;
