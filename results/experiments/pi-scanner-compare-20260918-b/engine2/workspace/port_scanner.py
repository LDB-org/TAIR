...
def scan_ports(host: str, ports: Sequence[int], timeout: float, workers: int) -> Tuple[List[int], List[int]]:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="TCP connect port scanner using only the Python 3 standard library."
    )
...
def main(argv: Sequence[str] = None) -> int:
    ...
    open_ports, closed_ports = scan_ports(host, ports, args.timeout, args.workers)
    result = {
        "host": host,
