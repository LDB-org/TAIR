...
if (!['engine', 'native'].includes(protocolMode)) throw new Error('Invalid --mode');
...
async function streamFn(_model, context, opts) {
  ...
  const result = ++turns <= 12 ? await remote({ ...context, tools: schemas, session_id: session, protocol_mode: protocolMode }, opts.signal)
...
if (!reply || (protocolMode === 'engine' && !summary.all_assistant_output_is_toolcall)) process.exitCode = 1;
