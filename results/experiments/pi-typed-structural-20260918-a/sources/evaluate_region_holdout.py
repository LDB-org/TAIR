...
def attempt(folder,payload,source,regions,pristine,check,case,mode,host,line_protocol=False,structural_protocol=False,compact_protocol=False,typed_protocol=False):
 ...
 try:
  ...
  if mode=='native':
   ...
   if typed_protocol:
    updated=compact.typed_decode(source,structural.digest(source),message.get('content') or '',choice['finish_reason'])
...
def main(out,host,repeats,line_protocol=False,structural_protocol=False,selected=None,semantic_targets=False,compact_protocol=False,typed_protocol=False):
 ...
 boundary=12 if typed_protocol else 10
 ...
 for index,(case,rep) in enumerate(jobs):
  ...
  for arm in (['native','hybrid'] if index%2==0 else ['hybrid','native']):
   ...
   for num,mode in enumerate(modes):
    r=attempt(folder/f'attempt{num}',payloads[mode],source,regions,pristine,check,case,mode,host,line_protocol,structural_protocol,compact_protocol,typed_protocol);attempts.append(r)
    if r['passed']:break
...
if __name__=='__main__':
 ...
 main(a.out,a.host,a.repeats,a.line_protocol,a.structural_protocol,a.cases,a.semantic_targets,a.compact_protocol,a.typed_protocol)
