...
expected={(a,c,k,r) for a in manifest['arms'] for c in manifest['cases'] for k in range(manifest['repeats']) for r in range(1,manifest.get('rounds_by_case',{}).get(c,manifest['rounds'])+1)}
...
auditpath=out/'repository-audit.json'
repoaudit={r['task']:r for r in json.loads(auditpath.read_text())} if auditpath.exists() else {}
for row in rows:
 row['raw_passed']=row['passed']
 key=f"{row['repeat']}-{row['case']}-{row['arm']}-{row['round']}"
 row['audited_behavior_passed']=row['validation']['passed']
 row['artifact_passed']=row['validation']['passed'] and row['test_integrity'] and not row['protected_changes']
 if repoaudit:
  row['passed'] &= repoaudit[key]['passed']
  row['artifact_passed'] &= repoaudit[key]['passed']
  row['audited_behavior_passed']=repoaudit[key]['behavior_passed']
 if row['case']=='summary_bug':
  ...
  row['artifact_passed'] &= review['requested_tests_effective'] is True

def summarize(group):
 records=[m for r in group for m in r['metrics']]
 return {'tasks':len(group),'passed':sum(r['passed'] for r in group),'artifact_passed':sum(r['artifact_passed'] for r in group),'raw_passed':sum(r['raw_passed'] for r in group),'seconds':sum(r['validated_seconds'] for r in group),
 'behavior_passed':sum(r['audited_behavior_passed'] for r in group),'edit_seconds':sum(m.get('wall_seconds',0) for m in records if m['action']=='edit'),'edit_calls':sum(m['action']=='edit' for m in records),'timeouts':sum(r['timed_out'] for r in group),'usage_complete':all(r['usage_complete'] for r in group),
...
for folder in out.glob('*/native-inference.jsonl'):
 ...
 prefix=manifest['configurations'][arm].get('TAIR_NATIVE_PREFIX_CACHE')=='1'
 if prefix:assert len({e['payload']['cache_salt'] for e in requests})==1
 native.append({'task':folder.parent.name,'requests':len(requests),'parallel_tool_calls':arm=='native_multi','workspace_prefix_cache':prefix})
