...
VERIFICATION_MARKER = '# TAIR requirement verification\n'


def needs_completion_verification(messages):
    calls, changed, verified = {}, False, False
    for message in messages:
        if message['role'] == 'user':
            changed, verified = False, False
        if message['role'] == 'assistant':
            calls.update({c['id']: c for c in message.get('content', [])
                          if isinstance(c, dict) and c.get('type') == 'toolCall'})
        if message['role'] != 'toolResult':
            continue
        call = calls.get(message.get('toolCallId'), {})
        if call.get('name') in ('edit', 'write', 'compact_edit', 'set_cli_default'):
            changed, verified = True, False
        if call.get('name') == 'bash' and call.get('arguments', {}).get('command', '').startswith(VERIFICATION_MARKER):
            verified = not message.get('isError', False)
    return changed and not verified


def verification_call(result):
    script = result['call']['arguments'].get('verification', '')
    try:
        tree = ast.parse(script)
        if not any(isinstance(node, ast.Assert) for node in ast.walk(tree)):
            raise ValueError('Verification must contain executable assert statements, not just print output.')
        compile(tree, '<requirement verification>', 'exec')
    except (SyntaxError, ValueError, TypeError) as error:
        script = 'raise AssertionError(' + repr(str(error)) + ')'
    trace_update(completion_verification_requested=True)
    command = VERIFICATION_MARKER + shlex.join([sys.executable, '-B', '-c', script])
    return {**result, 'call': {'name': 'bash', 'arguments': {'command': command}}}
...
def chat(payload):
    ...
    verify = (os.environ.get('PIJIT_VERIFY_COMPLETION') == '1' and
              any(t['name'] == 'bash' for t in tools) and needs_completion_verification(context['messages']))
    if verify:
        reply = tools[-1]
        reply['description'] = ('Propose completion and supply a Python verification script with assertions for the explicit '
            'requirements and edge cases. The script runs first; the answer is withheld until checks succeed. '
            'Do not modify project files in the verification script. Use temporary files for fixtures.')
        reply['parameters']['properties']['verification'] = {'type': 'string'}
        reply['parameters']['required'].append('verification')
    ...
    if os.environ.get('PIJIT_COMPLETION_CHECKS', '1') == '1':
    ...
    if verify and result['call']['name'] == 'reply_user':
        return verification_call(result)
