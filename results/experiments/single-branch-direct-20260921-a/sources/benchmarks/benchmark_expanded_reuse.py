...
def main(out, continuation=False, task_ids=None, fresh=False, local_tokenizer=None, verify_tokenizer=False, prefix_cache=False, compact_catalog=False, reuse_routing=False, ssh_compression=False, ablation=False, empty_book=False, success_reply_ablation=False, recovery_latest=False, recovery_ablation=False, capabilities_ablation=False, execution_ablation=False, reuse_review_ablation=False, write_references_ablation=False, recovery_admission_ablation=False, warm_reuse_ablation=False, repeat_count=1, argv_ablation=False, parallel_capabilities_ablation=False, reply_branch_ablation=False, combined_reuse_ablation=False, repair_feedback_ablation=False, dedup_descriptions_ablation=False, first_tool_ablation=False, first_tool_reuse_ablation=False):
    if first_tool_ablation and first_tool_reuse_ablation:raise ValueError("Choose one first-tool ablation")
    if (first_tool_ablation or first_tool_reuse_ablation) and any((repair_feedback_ablation, combined_reuse_ablation, reply_branch_ablation, parallel_capabilities_ablation, argv_ablation, warm_reuse_ablation, recovery_admission_ablation, write_references_ablation, reuse_review_ablation, execution_ablation, success_reply_ablation, recovery_ablation, capabilities_ablation, recovery_latest, reuse_routing, compact_catalog, dedup_descriptions_ablation)):
        raise ValueError('First-tool ablation must isolate native branch selection')
    if first_tool_ablation or first_tool_reuse_ablation:ablation=True
    if dedup_descriptions_ablation and any((repair_feedback_ablation, combined_reuse_ablation, reply_branch_ablation, parallel_capabilities_ablation, argv_ablation, warm_reuse_ablation, recovery_admission_ablation, write_references_ablation, reuse_review_ablation, execution_ablation, success_reply_ablation, recovery_ablation, capabilities_ablation, recovery_latest, reuse_routing, compact_catalog)):
        raise ValueError('Description ablation must isolate duplicated tool descriptions')
    if repair_feedback_ablation and any((combined_reuse_ablation, reply_branch_ablation, parallel_capabilities_ablation, argv_ablation, warm_reuse_ablation, recovery_admission_ablation, write_references_ablation, reuse_review_ablation, execution_ablation, success_reply_ablation, recovery_ablation, capabilities_ablation, recovery_latest, reuse_routing, compact_catalog)):
        raise ValueError('Repair feedback ablation must isolate negative feedback')
    if combined_reuse_ablation and any((reply_branch_ablation, parallel_capabilities_ablation, argv_ablation, warm_reuse_ablation, recovery_admission_ablation, write_references_ablation, reuse_review_ablation, execution_ablation, success_reply_ablation, recovery_ablation, capabilities_ablation, recovery_latest, reuse_routing, compact_catalog)):
        raise ValueError('Combined reuse ablation must isolate the combined configuration and reuse')
    if reply_branch_ablation and any((parallel_capabilities_ablation, argv_ablation, warm_reuse_ablation, recovery_admission_ablation, write_references_ablation, reuse_review_ablation, execution_ablation, success_reply_ablation, recovery_ablation, capabilities_ablation, recovery_latest, reuse_routing, compact_catalog)):
        raise ValueError('Reply branch ablation must isolate final-answer selection')
    if parallel_capabilities_ablation and any((argv_ablation, warm_reuse_ablation, recovery_admission_ablation, write_references_ablation, reuse_review_ablation, execution_ablation, success_reply_ablation, recovery_ablation, capabilities_ablation, recovery_latest, reuse_routing, compact_catalog)):
        raise ValueError('Parallel capabilities ablation must isolate negotiation scheduling')
    ...
    if success_reply_ablation or recovery_ablation or capabilities_ablation or execution_ablation or reuse_review_ablation or write_references_ablation or recovery_admission_ablation or warm_reuse_ablation or argv_ablation or parallel_capabilities_ablation or reply_branch_ablation or combined_reuse_ablation or repair_feedback_ablation or dedup_descriptions_ablation:
        ablation = True
    cli = shutil.which('pi')
    if not cli:
    ...
    jobs=cases()
    if repair_feedback_ablation:
        expanded=[]
        for job in jobs:
            expanded.append(job)
            if job['id'] in ('jsonl_changed','unique_changed'):
                repeat=copy.deepcopy(job)
                repeat.update(id=job['id']+'_again',phase='repeat')
    ...
    if parallel_capabilities_ablation:
        configured_arms=['native','tair_no_reuse','tair_parallel_caps']
    if reply_branch_ablation:
        configured_arms=['native','tair_no_reuse','tair_reply_branch']
    if combined_reuse_ablation:
        configured_arms=['native','tair_no_reuse','tair_combined_no_reuse','tair_combined']
    if repair_feedback_ablation:
        configured_arms=['native','tair_combined','tair_feedback']
    if dedup_descriptions_ablation:
        configured_arms=['native','tair_no_reuse','tair_dedup']
    if first_tool_ablation:configured_arms=['native','tair_no_reuse','tair_general']
    if first_tool_reuse_ablation:configured_arms=['native','tair','tair_general','tair_general_reuse']
    ...
    manifest['reuse_ablation'] = ablation and not first_tool_ablation and not first_tool_reuse_ablation and not success_reply_ablation and not recovery_ablation and not capabilities_ablation and not execution_ablation and not reuse_review_ablation and not write_references_ablation and not recovery_admission_ablation and not warm_reuse_ablation and not argv_ablation and not parallel_capabilities_ablation and not reply_branch_ablation and not combined_reuse_ablation and not repair_feedback_ablation and not dedup_descriptions_ablation
    manifest['dedup_descriptions_ablation'] = dedup_descriptions_ablation
    manifest['first_tool_ablation'] = first_tool_ablation
    manifest['first_tool_reuse_ablation'] = first_tool_reuse_ablation
    manifest['repair_feedback_ablation'] = repair_feedback_ablation
    manifest['combined_reuse_ablation'] = combined_reuse_ablation
    manifest['reply_branch_ablation'] = reply_branch_ablation
    manifest['parallel_capabilities_ablation'] = parallel_capabilities_ablation
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='tair-expanded-project-') as temp:
            ...
            for task_id,arm in schedule:
                ...
                overrides['PIJIT_PLAN_DISABLE_REUSE']='1' if arm in ('tair_no_reuse','tair_reply','tair_recovery','tair_capcache','tair_argv','tair_parallel_caps','tair_reply_branch','tair_combined_no_reuse','tair_dedup','tair_general') else '0'
                overrides['PIJIT_PLAN_SUCCESS_REPLY']='1' if arm=='tair_reply' else '0'
                combined_for_arm = arm in ('tair_combined_no_reuse','tair_combined','tair_feedback')
                reply_for_arm = arm=='tair_reply_branch' or combined_for_arm
                argv_for_arm = arm=='tair_argv' or combined_for_arm
                references_for_arm = arm in ('tair_refs','tair_recovered') or warm_reuse_ablation and arm!='native' or combined_for_arm
                admission_for_arm = arm=='tair_recovered' or warm_reuse_ablation and arm!='native' or combined_for_arm
                overrides['PIJIT_FIRST_TOOL_CLASSIFICATION']='0' if arm in ('tair_general','tair_general_reuse') else '1'
                overrides['PIJIT_DEDUP_TOOL_DESCRIPTIONS']='1' if arm=='tair_dedup' else '0'
                overrides['PIJIT_REPAIR_FEEDBACK']='1' if arm=='tair_feedback' else '0'
                overrides['PIJIT_REPLY_BRANCH']='1' if reply_for_arm else '0'
                overrides['PIJIT_PARALLEL_CAPABILITIES']='1' if arm=='tair_parallel_caps' else '0'
                overrides['PIJIT_BASH_ARGV']='1' if argv_for_arm else '0'
                ...
                try:
                    ...
                    apply_workflow_result(row,validate_workflow(job,events,project))
                    ...
                    if arm!='native':
                        ...
                        assert chats and all(m.get('plan_reuse_enabled')==(arm not in ('tair_no_reuse','tair_reply','tair_recovery','tair_capcache','tair_argv','tair_parallel_caps','tair_reply_branch','tair_combined_no_reuse','tair_dedup','tair_general')) for m in chats), 'Runtime reuse configuration mismatch'
                        ...
                        assert all(m.get('first_tool_classification_enabled')==(arm not in ('tair_general','tair_general_reuse')) for m in chats), 'Runtime first-tool classification mismatch'
                        assert all(m.get('dedup_tool_descriptions')==(arm=='tair_dedup') for m in chats), 'Runtime tool-description dedup mismatch'
                        assert all(m.get('repair_feedback_enabled')==(arm=='tair_feedback') for m in chats), 'Runtime repair feedback mismatch'
                        assert all(m.get('reply_branch_enabled')==(reply_for_arm and not m.get('recovery_planning')) for m in chats), 'Runtime reply branch mismatch'
                        assert all(m.get('parallel_capabilities_enabled')==(arm=='tair_parallel_caps') for m in chats), 'Runtime capability scheduling mismatch'
                        assert all(m.get('bash_argv_enabled')==argv_for_arm for m in chats), 'Runtime bash argv mismatch'
                ...
                print(json.dumps({k:v for k,v in row.items() if k in ['task','arm','passed','artifact_passed','workflow_validation','seconds','usage_complete','reuse_executed','tool_errors','known_generated_argument_tokens','inference_requests','harness_error']}),flush=True)
...
if __name__=='__main__':
    ...
    p.add_argument('--dedup-descriptions-ablation',action='store_true')
    p.add_argument('--first-tool-ablation',action='store_true')
    p.add_argument('--first-tool-reuse-ablation',action='store_true')
    p.add_argument('--repair-feedback-ablation',action='store_true')
    p.add_argument('--combined-reuse-ablation',action='store_true')
    p.add_argument('--reply-branch-ablation',action='store_true')
    p.add_argument('--parallel-capabilities-ablation',action='store_true')
    p.add_argument('--argv-ablation',action='store_true')
    p.add_argument('--repeat-count',type=int,default=1)
    args=p.parse_args();main(args.out,args.continuation,args.tasks,args.fresh,args.local_tokenizer,args.verify_tokenizer,args.prefix_cache,args.compact_catalog,args.reuse_routing,args.ssh_compression,args.ablation,args.empty_book,args.success_reply_ablation,args.recovery_latest,args.recovery_ablation,args.capabilities_ablation,args.execution_ablation,args.reuse_review_ablation,args.write_references_ablation,args.recovery_admission_ablation,args.warm_reuse_ablation,args.repeat_count,args.argv_ablation,args.parallel_capabilities_ablation,args.reply_branch_ablation,args.combined_reuse_ablation,args.repair_feedback_ablation,args.dedup_descriptions_ablation,args.first_tool_ablation,args.first_tool_reuse_ablation)
