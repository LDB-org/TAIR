...
def main():
    ...
    parser.add_argument("--kv-cache-memory-bytes", type=int, help="Explicit bounded KV allocation instead of automatic profiling")
    args = parser.parse_args()
    if (args.output.exists() or args.repeats < 1 or not 0 < args.gpu_memory_utilization < 1
            or args.max_model_len < 1 or args.max_batched_tokens < 1):
        parser.error("Require a new output path, positive repeats, and GPU fraction in (0, 1)")
    ...
    settings = dict(dtype="bfloat16", max_model_len=args.max_model_len,
                    max_num_batched_tokens=args.max_batched_tokens, max_num_seqs=1,
    ...
    if args.kv_cache_memory_bytes is not None:
        if args.kv_cache_memory_bytes <= 0:
            parser.error("KV cache bytes must be positive")
        settings["kv_cache_memory_bytes"] = args.kv_cache_memory_bytes
