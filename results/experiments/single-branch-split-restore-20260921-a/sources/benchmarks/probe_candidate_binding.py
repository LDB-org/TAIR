...
def main(out,tokenizer):
    ...
    manifest=dict(fixtures=fixtures,jobs={key:jobs[key] for key in ['unique_repeat','unique_changed']},entries=entries,
    ...
    with (out/'ssh-stderr.log').open('w') as log:
        ...
        try:
            ...
            for number,fixture in enumerate(fixtures):
                ...
                old_options=old.branches(tools,candidates);new_options=bridge.tool_plan.branches(tools,candidates)
                ...
                for updated in ([False,True] if number%2==0 else [True,False]):
                    module=bridge.tool_plan if updated else old;options=new_options if updated else old_options
                    ...
                    try:
                        response=bridge.infer(messages,[dict(options[selected],parameters=module.decoder_schema(options[selected]['parameters']))],
                            ...
                            branch_instruction=lambda index,option:module.continuation(selected,tools,candidates,options[selected]))
...
if __name__=='__main__':
    ...
    parser.add_argument('--tokenizer',type=Path,required=True);args=parser.parse_args();main(args.out,args.tokenizer)
