def merge_intervals(items):
    ...
    Returns:
        A new sorted list of merged two-element lists. The input list and
        its sublists are not mutated, and no mutable sublists are shared
        with the input.
    """
    if not items:
        return []
    ...
    for interval in sorted_items:
        ...
        if not merged:
            ...
            last = merged[-1]
            # Overlap or touch: current start <= last end
            if start <= last[1]:
                if end > last[1]:
                    last[1] = end
            else:
