...
def main(out, continuation=False, task_ids=None, fresh=False, local_tokenizer=None, verify_tokenizer=False, prefix_cache=False, compact_catalog=False, reuse_routing=False, ssh_compression=False, ablation=False, empty_book=False):
    cli = shutil.which('pi')
    if not cli:
    ...
    manifest['reuse_ablation'] = ablation
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='tair-expanded-project-') as temp:
            ...
            for task_id,arm in schedule:
                ...
                overrides['PIJIT_PLAN_DISABLE_REUSE']='1' if arm=='tair_no_reuse' else '0'
                try:
                    ...
                    if arm!='native':
                        ...
                        assert chats and all(m.get('plan_reuse_enabled')==(arm!='tair_no_reuse') for m in chats), 'Runtime reuse configuration mismatch'
...
if __name__=='__main__':
    ...
    args=p.parse_args();main(args.out,args.continuation,args.tasks,args.fresh,args.local_tokenizer,args.verify_tokenizer,args.prefix_cache,args.compact_catalog,args.reuse_routing,args.ssh_compression,args.ablation,args.empty_book)
