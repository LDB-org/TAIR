...
class PlanBook:
    ...
    @staticmethod
    def insert(db, contract, source, evidence):
    ...
    def candidates(self, task, contracts, limit=15):
        ...
        with self.connection() as db:
            rows = db.execute(f'''SELECT e.* FROM search JOIN entries e ON e.seq=search.rowid
                ...
                LIMIT ?''', [query, *hashes, limit])
            return [checked(row) for row in rows]
