...
def main(args):
    ...
    manifest = {'arms': args.arms, 'cases': list(scenarios), 'rounds': args.rounds, 'repeats': args.repeats,
        ...
        'method': 'No extra completion policy or verification gate. Native single/multi differ only in parallel_tool_calls, and both permit workspace-scoped prefix caching. Hybrid retains native tool-call history and enables local bound codebook reuse; hybrid no_book/on differ only in disable-codebook. TAIR legacy keeps its classified batch planner and per-request cache salt. Sequential interleaving, no benchmark retries; all failures retained. Fresh per-arm state, restored source for warm rounds, external oracle. Shared server and finite development tasks; no generic production claim.'}
