...
def test_v2_mixed_batch_preserves_promoted_sampler_logits(monkeypatch, dtype):
    import numpy as np
    monkeypatch.setattr(module, 'audit', lambda *args, **kwargs: None)
    logits = torch.tensor([[100., 3., 7.], [4., 6., 8.]], dtype=dtype)
    ordinary = torch.tensor([[4.0001, 6.0001, 8.0001]], dtype=torch.float32)
    ...
    class Sampler:
        ...
        def sample(self, *args, **kwargs):
            return torch.tensor([2]), ordinary

    mapping = torch.tensor([5, 9])
    selected, processed = module.classify_v2(Sampler(), logits, mapping, mapping,
        np.array([5, 9]), mapping, mapping, mapping, return_logprobs=True)
    assert selected.tolist() == [2, 2]
    assert processed.dtype == torch.float32
    assert torch.equal(processed[1], ordinary[0])
    assert torch.equal(processed[0], logits[0].float())
