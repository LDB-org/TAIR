...
@stage('generation')
def infer(messages, tools, instruction=None, classification_prompt=None):
...
def generic_plan_chat(payload):
    ...
    policy = (context.get('systemPrompt', '') +
              ...
              'Reuse content only when its FULL bytes match the current requirement; stored content is only '
...
def reject_repeated_plan(call, messages):
    ...
    for message in messages[latest+1:]:
        ...
        elif message['role']=='toolResult' and not message.get('isError'):
            if previous.get(message.get('toolCallId')) == call['arguments']:
                raise ValueError('Duplicate completed plan rejected; no tools executed again')
...
def complete_tool_plan(payload):
    ...
    return dict(admitted=admitted, cache_hit=bool(reused), validation='tool execution only; not semantic verification')
