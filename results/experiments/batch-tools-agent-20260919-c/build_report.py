...
'|---|---:|---:|---:|---:|---:|---:|---:|---:|']
for arm in ['native','optimized','batched']:
...
'## Scenario totals','','| Scenario | Native seconds / requests | Optimized single-step seconds / requests | Batched seconds / requests |','|---|---:|---:|---:|']
for case in prof['native']['by_case']:
 lines.append('| '+case+' | '+' | '.join(f"{prof[a]['by_case'][case]['seconds']:.2f} / {prof[a]['by_case'][case]['requests']}" for a in ['native','optimized','batched'])+' |')
lines+=['','## Failures','']
