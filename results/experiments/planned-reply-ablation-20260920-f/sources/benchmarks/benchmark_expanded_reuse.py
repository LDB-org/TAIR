...
def main(out, continuation=False, task_ids=None, fresh=False, local_tokenizer=None, verify_tokenizer=False, prefix_cache=False, compact_catalog=False, reuse_routing=False, ssh_compression=False, ablation=False, empty_book=False, success_reply_ablation=False, recovery_latest=False):
    ...
    manifest['reuse_ablation'] = ablation and not success_reply_ablation
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='tair-expanded-project-') as temp:
            ...
            for task_id,arm in schedule:
                ...
                overrides['PIJIT_PLAN_DISABLE_REUSE']='1' if arm in ('tair_no_reuse','tair_reply') else '0'
                overrides['PIJIT_PLAN_SUCCESS_REPLY']='1' if arm=='tair_reply' else '0'
                overrides['PIJIT_RECOVERY_LATEST']='1' if arm!='native' and recovery_latest else '0'
                try:
                    ...
                    if arm!='native':
                        ...
                        assert chats and all(m.get('plan_reuse_enabled')==(arm not in ('tair_no_reuse','tair_reply')) for m in chats), 'Runtime reuse configuration mismatch'
                        assert all(m.get('plan_success_reply_enabled')==(arm=='tair_reply') for m in chats), 'Runtime reply configuration mismatch'
                        assert all(m.get('recovery_scope')==('latest_result' if recovery_latest else 'whole_user_turn') for m in chats), 'Runtime recovery configuration mismatch'
...
if __name__=='__main__':
    ...
    args=p.parse_args();main(args.out,args.continuation,args.tasks,args.fresh,args.local_tokenizer,args.verify_tokenizer,args.prefix_cache,args.compact_catalog,args.reuse_routing,args.ssh_compression,args.ablation,args.empty_book,args.success_reply_ablation,args.recovery_latest)
