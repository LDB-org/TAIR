...
import { PlanReply } from './plan_reply.mjs';
import { CapabilitiesCache } from './capabilities_cache.mjs';
...
export default function(pi: any) {
  ...
  const planReply = new PlanReply();
  pi.on('before_agent_start', () => { planReply.clear(); });
  ...
  pi.on('session_start', async (_event: any, ctx: any) => {
    ...
    pendingPlans.clear(); planReply.clear();
  ...
  pi.registerProvider('pijit', {
    ...
    streamSimple(model: any, context: any, options: any) {
      ...
      (async () => {
        ...
        try {
          const preparedReply = planReply.take(context.messages, sessionId, options?.signal?.aborted);
          const cacheCapabilities = toolPlan && process.env.PIJIT_CAPABILITIES_CACHE === '1';
          const capabilitiesKey = createHash('sha256').update(JSON.stringify([
            process.env.PIJIT_URL, process.env.PIJIT_MODEL, process.env.PIJIT_API_KEY])).digest('hex');
          const result = preparedReply === undefined
            ? await bridge({ action: 'chat', context, cwd, session_id: sessionId,
                ...(cacheCapabilities ? { server_capabilities: capabilitiesCache.get(capabilitiesKey, sessionId) } : {}),
                ...(toolPlan ? { inner_tools: innerCatalog } : {}) }, options?.signal)
            : { call: { name: 'reply_user', arguments: { content: preparedReply } } };
          if (cacheCapabilities && result.server_capabilities)
            capabilitiesCache.remember(capabilitiesKey, sessionId, result.server_capabilities);
          if (preparedReply !== undefined) appendPlanEvent(process.env.PIJIT_STATE_DIR!, cwd, sessionId,
            context.messages.at(-1).toolCallId, { event: 'planned_reply_delivered', inference_requests: 0 });
          record(result);
          ...
          if (toolPlan && call.name === 'plan') pendingPlans.set(result.request_id,
            { ...result, replyHistory: structuredClone(context.messages) });
  ...
  if (toolPlan) pi.registerTool({
    ...
    async execute(id: string, args: any, signal: AbortSignal, onUpdate: any, ctx: any) {
      ...
      if (pending?.plan_task) {
        try {
          learned = await bridge({ action: 'tool_plan_complete', cwd: ctx.cwd, steps: args.steps,
            ...
            reused_template_ids: pending.reused_template_ids,
      ...
      if (!signal?.aborted && pending?.on_success_reply && process.env.PIJIT_PLAN_SUCCESS_REPLY === '1') {
        const offered = planReply.offer(id, pending.on_success_reply, pending.replyHistory, args.steps,
          results, ctx.sessionManager.getSessionId(), pending.expected_plan_outputs);
        log({ event: 'planned_reply_offer', eligible: offered, reason: planReply.rejectionReason || null });
      } else if (process.env.PIJIT_PLAN_SUCCESS_REPLY === '1') {
        log({ event: 'planned_reply_skip', reason: signal?.aborted ? 'cancelled'
          : pending?.on_success_reply === '' ? 'model_requested_continuation' : 'missing_completion_metadata' });
      }
