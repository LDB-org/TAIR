...
def parse_port_spec(spec: str) -> List[int]:
    ...
    Accepts individual ports, inclusive ranges (e.g. 80,443,8000-8010),
    and inclusive stepped ranges (e.g. 20-26/2 gives 20,22,24,26).
    ...
    for raw_part in spec.split(","):
        ...
        if "/" in part:
            ...
            if not range_part or not step_part:
                raise ValueError(f"malformed port range: {part!r}")
            if "-" not in range_part:
                raise ValueError(f"step only allowed on ranges: {part!r}")
