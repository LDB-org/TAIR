            ...
            try:
                ...
                if route == '/v1/openjev/toolcall' and 'generated_argument_tokens' in failure:
                    ...
                    record.update(generated_argument_tokens=failure['generated_argument_tokens'],
                                  ...
                                  engine_request_id=failure.get('request_id'), engine_decision_index=selected,
                                  engine_finish_reason=failure.get('finish_reason'),
                                  engine_generation_diagnostics=failure.get('generation_diagnostics'))
                    trace_update(plan_token_budget=failure.get('plan_budget'))
                    if error.code == 500 and failure.get('finish_reason') == 'length':
                        raise GenerationLengthError('generation reached its output limit; no executable call was returned') from error
...
@stage('generation')
def infer(messages, tools, instruction=None, classification_prompt=None, branch_instruction=None, plan_budget=False, budget_resolver=None):
    ...
    with stage('generation_preparation'):
        if not classification_prompt:
            ...
            classification_prompt = 'Choose the single next tool needed for the current task. Reply only with its letter.\n' + options
        messages = messages + [{'role': 'user', 'content': classification_prompt}]
        tails = []
        ...
        if os.environ.get('PIJIT_SERIAL_PREPARATION') == '1':
            if budget_resolver:
                plan_budget = budget_resolver()
            prefix, suffixes = prepare_continuations(messages, tails)
            candidate_ids = labels(len(tools))
        else:
            work = [lambda: prepare_continuations(messages, tails), lambda: labels(len(tools))]
            if budget_resolver:
                work.append(budget_resolver)
            prepared, candidate_ids, *budget = parallel(lambda task: task(), work, max_workers=len(work))
            if budget:
...
def recovered_mutation_steps(messages):
    """Completed mutations from failed plans, awaiting a successful recovery check."""
    latest_user = max((i for i, m in enumerate(messages) if m['role']=='user'), default=-1)
    calls, pending = {}, {}
    checked = False
    for message in messages[latest_user+1:]:
        if message['role']=='assistant':
            for block in message.get('content', []):
                if isinstance(block, dict) and block.get('type')=='toolCall' and block.get('name')=='plan':
                    calls[block['id']] = block.get('arguments', {}).get('steps', [])
        if message['role']!='toolResult':
            continue
        checked = False
        steps = calls.pop(message.get('toolCallId'), None)
        if not steps or message.get('toolName')!='plan':
            return []
        try:
            result = json.loads(text_content(message.get('content')))
            failed = bool(message.get('isError'))
            completed = result['completed' if failed else 'results']
            count = result['failed_step'] if failed else len(steps)
            if type(count) is not int or not 0 <= count <= len(steps) or failed and count==len(steps):
                return []
            if len(completed)!=count or any(r['name']!=s['name'] for r,s in zip(completed, steps)):
                return []
            for step in steps[:count]:
                if step['name'] in ('write', 'edit'):
                    ...
                    if failed:
                        ...
                        pending.pop(path, None)  # Successful mutation plans already use normal admission.
            checked = not failed and any(step['name']=='bash' for step in steps)
        except (ValueError, TypeError, KeyError, AttributeError):
            return []
    return list(pending.values()) if checked and not calls else []


def admit_recovered_mutations(payload, task):
    steps = recovered_mutation_steps(payload['context']['messages'])
    if not steps:
        return None
    child_trace = {}
    token = TRACE.set(child_trace)
    try:
        learned = complete_tool_plan(dict(cwd=payload['cwd'], task=task, steps=steps, recovery_admission=True))
        return dict(learned, observed=child_trace.get('final_content_observed', 0),
                    skipped=child_trace.get('final_content_skipped', []))
    except Exception as error:
        return dict(admitted=[], cache_error=str(error))
    finally:
        TRACE.reset(token)


def record_repaired_content(payload, task):
    """Reject exact old bytes for this task after an observed failed-check repair."""
    messages = payload['context']['messages']
    start = max((i for i,m in enumerate(messages) if m['role']=='user'), default=-1)
    cwd = Path(payload['cwd']).resolve()
    calls, failed_writes, corrected = {}, {}, set()
    checked = False
    def local_path(name):
        path = (cwd/name).resolve()
        return path if path.is_relative_to(cwd) else None
    for message in messages[start+1:]:
        if message['role']=='assistant':
            for block in message.get('content', []):
                if isinstance(block,dict) and block.get('type')=='toolCall' and block.get('name')=='plan':
        ...
        try:
            ...
            if type(count) is not int or not 0<=count<=len(steps) or failed and count==len(steps):
                return []
            if len(completed)!=count or any(a['name']!=b['name'] for a,b in zip(completed,steps)):
                return []
            for step in steps[:count]:
                if step['name'] not in ('write','edit'):
                    continue
                path = local_path(step['arguments']['path'])
                if path is None:
                    continue
                if failed:
                    corrected.discard(path)
                    if step['name']=='write' and steps[count]['name']=='bash':
                        failed_writes.setdefault(path,[]).append(step['arguments']['content'])
                elif path in failed_writes:
                    corrected.add(path)
            checked = not failed and any(step['name']=='bash' for step in steps)
        except (ValueError,TypeError,KeyError,AttributeError):
            return []
    if not checked or calls:
        return []
    book = tool_plan.ToolContentBook(paths(str(cwd))/'tool-plan-codebook.sqlite3')
    records = []
    for path in corrected:
        ...
        for previous in dict.fromkeys(failed_writes[path]):
            if previous==current:
                continue
            identities = book.reject_content(previous,task)
            if identities:
                records.append(dict(path=str(path.relative_to(cwd)),rejected_ids=identities,
                                    reason='failed_check_then_replaced_content; semantic_correctness_unverified'))
    return records
...
def generic_plan_chat(payload):
    ...
    parallel_capabilities = os.environ.get('PIJIT_PARALLEL_CAPABILITIES') == '1'
    trace_update(parallel_capabilities_enabled=parallel_capabilities)
    if not parallel_capabilities:
        negotiate_budget()
    context = payload['context']
    repair_feedback = os.environ.get('PIJIT_REPAIR_FEEDBACK') == '1'
    trace_update(repair_feedback_enabled=repair_feedback)
    recovery_admission = os.environ.get('PIJIT_RECOVERY_ADMISSION') == '1'
    trace_update(recovery_admission_enabled=recovery_admission)
    ...
    recovery_default = '1' if os.environ.get('PIJIT_PLAN_SUCCESS_REPLY') == '1' else '0'
    recovery_latest = os.environ.get('PIJIT_RECOVERY_LATEST', recovery_default) == '1'
    ...
    def retrieve(content_book, template=False):
        compatible_payload = tool_plan.payload_guard(task)
        def eligible(entry):
            nonlocal payload_mismatches
            compatible = compatible_payload(dict(entry, plan_template=template))
            payload_mismatches += not compatible
            return compatible
        return content_book.candidates(task, {}, limit=candidate_limit, eligible=eligible,
                                       repair_feedback=repair_feedback)
    can_reuse_write = reuse_enabled and not recovery and any(t['name']=='write' for t in tools)
    trace_update(candidate_retrieval_skipped='recovery' if recovery else None)
    ...
    trace_update(template_entries=template_book.count() if template_book else
                 None if recovery and os.environ.get('PIJIT_PLAN_TEMPLATES') == '1' else 0)
    ...
    reuse_review = os.environ.get('PIJIT_REUSE_REVIEW') == '1'
    trace_update(reuse_review_enabled=reuse_review)
    write_references = os.environ.get('PIJIT_WRITE_REFERENCES') == '1'
    trace_update(write_references_enabled=write_references)
    bash_argv = os.environ.get('PIJIT_BASH_ARGV') == '1'
    trace_update(bash_argv_enabled=bash_argv)
    reply_branch = os.environ.get('PIJIT_REPLY_BRANCH') == '1' and not recovery
    trace_update(reply_branch_enabled=reply_branch)
    options = tool_plan.branches(tools, candidates, success_reply=success_reply,
                                 reuse_review=reuse_review, write_references=write_references, bash_argv=bash_argv, reply_branch=reply_branch)
    if recovery:
        options = [tool_plan.recovery_option(options[-1])]
    decode_options = options
    selection_indices = list(range(len(options)))
    classify_first_tool = os.environ.get('PIJIT_FIRST_TOOL_CLASSIFICATION') != '0'
    trace_update(first_tool_classification_enabled=classify_first_tool)
    if (reuse_routing or not classify_first_tool) and not recovery:
    ...
    if bash_argv:
        policy += ('\nFor a single program invocation, bash.command may be an array of argument strings, '
                   'for example ["node", "-e", "JavaScript source"] or ["python3", "-c", "Python source"]. '
                   'Prefer this form for inline code or arguments containing quotes. The runtime quotes each literal argument; '
                   'do not add shell quoting to array elements. Arrays do not expand variables, globs or operators. '
                   'Use the ordinary command string for pipelines, redirects or compound shell commands.')
    execution_guidance = os.environ.get('PIJIT_PLAN_EXECUTION_GUIDANCE') == '1'
    trace_update(execution_guidance_enabled=execution_guidance)
    if execution_guidance:
        policy += ('\nBefore emitting a command, account for its execution prerequisites: required fixtures and resources, '
                   'working directory, relative path resolution, and shell quoting. Set up known prerequisites before '
                   'dependent checks in the same plan; inspect unknown state before relying on it. '
                   'Use isolated test fixtures instead of assuming external state exists. '
                   'Preserve the user-requested operation order and all required checks. Do not suppress errors or weaken assertions '
                   'to make a check pass. No extra explanatory output is needed for this preparation.')
    if success_reply:
    ...
    if write_references and candidates:
        policy += ('\nAny write, including first or later steps, may supply content={"stored": N} instead of a string. '
                   'N is the stored index printed in the current catalog; the runtime expands the exact content. '
                   'Use this only if that content satisfies ALL current requirements. Otherwise generate a normal string. '
                   'This does not reuse check results; execute required checks normally.')
    dedup_tool_descriptions = os.environ.get('PIJIT_DEDUP_TOOL_DESCRIPTIONS') == '1'
    trace_update(dedup_tool_descriptions=dedup_tool_descriptions)
    descriptions = [('First action '+tools[index]['name']+'. See INNER TOOLS above.'
                     if dedup_tool_descriptions and not recovery and index<len(tools) else option['description'])
                    for index,option in zip(selection_indices,options)]
    ...
    response = infer(messages, [dict(option, parameters=tool_plan.decoder_schema(option['parameters'])) for option in options],
                     ...
                     **(dict(budget_resolver=negotiate_budget) if parallel_capabilities else {}),
                     ...
                     'choose '+LABELS[len(options)-2 if reply_branch else len(options)-1]+' to reply. Do NOT repeat completed actions. '
                     ...
                     'Stored catalog entries are reference data, NOT additional tasks.' if candidates else '')))
    ...
    branch = ('recovery' if recovery else 'tool:'+tools[index]['name'] if 0 <= index < len(tools) else
              'reuse_content' if len(tools) <= index < len(tools)+len(candidates) else
              'reply' if reply_branch and index==len(tools)+len(candidates) else 'general')
    ...
    wire_arguments = response['call']['arguments']
    wire_steps = wire_arguments.get('steps', wire_arguments.get('rest', []))
    trace_update(bash_argv_steps=sum(isinstance(args.get('command'), list) for args in
                 [wire_arguments.get('first', {}), *[step.get('arguments', {}) for step in wire_steps]]))
    ...
    if repair_feedback and call['name']=='reply_user':
        try:
            with stage('repair_feedback'):
                trace_update(repair_feedback=record_repaired_content(payload,task))
        except Exception as error:
            trace_update(repair_feedback_error=str(error))
    if recovery_admission and call['name']=='reply_user':
        with stage('recovery_admission'):
            learned = admit_recovered_mutations(payload, task)
        trace_update(recovery_admission=learned)
        if learned:
            response = dict(response, admission_count=len(learned['admitted']))
...
def final_mutated_contents(payload):
    cwd = Path(payload['cwd']).resolve()
    contents, skipped = {}, []
    for name in dict.fromkeys(s['arguments']['path'] for s in payload['steps'] if s['name'] in ('write', 'edit')):
        try:
            path = (cwd/name).resolve()
            ...
            if len(data) > 1024*1024:
                raise ValueError('content exceeds admission read limit')
            contents[name] = data.decode('utf-8')
        except (OSError, ValueError) as error:
            skipped.append(dict(path=name, reason=str(error)))
    trace_update(final_content_observed=len(contents), final_content_skipped=skipped)
    return contents
...
def complete_tool_plan(payload):
    ...
    book = tool_plan.ToolContentBook(paths(payload['cwd'])/'tool-plan-codebook.sqlite3')
    final_contents = final_mutated_contents(payload)
    observed = set(final_contents.values())
    reused = payload.get('reused_content_ids', [])
    reused_sources = set()
    if reused:
        with book.connection() as db:
            entries = list(db.execute(
                'SELECT id,source FROM entries WHERE id IN ('+','.join('?' for _ in reused)+')', reused))
            reused = [row[0] for row in entries if row[1] in observed]
            reused_sources = {row[1] for row in entries if row[1] in observed}
    reused_templates = payload.get('reused_template_ids', [])
    templates = None
    if os.environ.get('PIJIT_PLAN_TEMPLATES') == '1' and not payload.get('recovery_admission'):
        templates = tool_plan.ToolContentBook(paths(payload['cwd'])/'tool-plan-templates.sqlite3')
        if reused_templates:
            with templates.connection() as db:
                entries = list(db.execute(
                    'SELECT id,source FROM entries WHERE id IN ('+','.join('?' for _ in reused_templates)+')', reused_templates))
                reused_templates = [row[0] for row in entries if json.loads(row[1])['content'] in observed]
                reused_sources.update(json.loads(row[1])['content'] for row in entries if row[0] in reused_templates)
    contents = list(dict.fromkeys(content for content in final_contents.values() if content not in reused_sources))
    admitted = book.admit([(payload['task'], content, tool_plan.content_evidence(
        str((Path(payload['cwd'])/name).resolve().relative_to(Path(payload['cwd']).resolve()))
        for name, observed_content in final_contents.items() if observed_content==content))
    ...
    if templates is not None:
        ...
        if source and json.loads(source)['content'] in observed and not reused_templates:
    ...
    mutations = {s['name'] for s in payload['steps']} & {'write', 'edit'}
    learned_reason = 'new_write_content' if 'write' in mutations else 'new_edit_content'
    return dict(admitted=admitted, cache_hit=bool(reused or reused_templates), admission_count=len(admitted),
                ...
                admission_reason=learned_reason if admitted else 'deduplicated_or_reused' if final_contents else
                'no_eligible_final_content' if mutations else 'no_write_steps',
