...
def main(out, tokenizer, open_content=False, split_shift=0):
    ...
    manifest.update(open_content=open_content,split_shift=split_shift)
    if open_content:
        ...
        try:
            ...
            for index,args in enumerate(fixtures):
                ...
                if split_shift:
                    ...
                    boundary=len(prefix)+split_shift
                    assert 0<boundary<len(full)
                    prefix,suffixes=full[:boundary],[full[boundary:]]
...
if __name__=='__main__':
    ...
    parser.add_argument('--split-shift',type=int,default=0,
        help='Move the singleton classification boundary while keeping full generation token IDs identical.')
    args=parser.parse_args(); main(args.out,args.tokenizer,args.open_content,args.split_shift)
