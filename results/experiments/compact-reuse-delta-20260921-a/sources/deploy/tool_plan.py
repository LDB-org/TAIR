...
def compatible_payload(entry, task):
    return payload_guard(task)(entry)
...
def branches(tools, candidates, success_reply=False, reuse_review=False, write_references=False, bash_argv=False, reply_branch=False):
...
def decode(response, tools, candidates, options):
    ...
    if index == len(options)-1:
        ...
        steps = copy.deepcopy([dict(name=tools[index]['name'], arguments=arguments['first']), *arguments['rest']])
    else:
