...
run='request-delta-template-guards-20260920-a';out=ROOT/'results/experiments'/run;out.mkdir(exist_ok=False)
...
env.update(PIJIT_PLAN_TEMPLATES='1',PIJIT_STATE_DIR=str(state),PIJIT_SSH_HOST='rs-yuesheng-gpu-public',PIJIT_PYTHON=sys.executable)
shutil.copyfile(__file__,out/'runner.py')
before=inventory();(out/'book-before.json').write_text(json.dumps(before,indent=2));rows=[]
try:
 for number,(kind,spec) in enumerate(jobs[:1]*2,1):
  ...
  expected=(ROOT/'benchmarks/fixtures/file_io_long.py').read_text() + ('\nTEMPLATE_VARIANT = 2\n' if number==1 else '')
  if number==2:
   import shlex
   command='python3 -c '+shlex.quote(f'from pathlib import Path; Path({str(dest/"receipt.txt")!r}).write_text("checked")')
   prompt += ' After compilation, also run this separate bash command: '+command+'. This additional check is mandatory.'
  prompt += '\nThe module must contain exactly the following source, byte for byte:\n```python\n' + expected + '```'
  ...
  try:
   ...
   try:
    ...
    if number==2:assert (dest/'receipt.txt').read_text()=='checked'
    row['correct']=True
  ...
  row.update(template_hits=sum(bool(r.get('reused_template_ids')) for r in chats),template_admissions=sum(len(r.get('template_admitted',[])) for r in completions));row.update(book_entries_before=len(round_before),book_entries_after=len(inventory()),book_after=inventory());rows.append(row);(out/'report.json').write_text(json.dumps(rows,indent=2));print(json.dumps({k:v for k,v in row.items() if k not in ('records','book_after')}),flush=True)
finally:
