...
def attempt(folder,payload,source,regions,pristine,check,case,mode,host,line_protocol=False,structural_protocol=False,compact_protocol=False):
...
def main(out,host,repeats,line_protocol=False,structural_protocol=False,selected=None,semantic_targets=False,compact_protocol=False):
 ...
 if compact_protocol:
  templates['multi']['structured_outputs']={'regex':compact.grammar(source)}
  templates['multi']['messages'][0]['content']=compact.prompt(source)
 ...
 manifest['previously_evaluated_cases']=[k for k in cases if k in list(CASES)[:10]]
 manifest['new_cases']=[k for k in cases if k in list(CASES)[10:]]
 ...
 for index,(case,rep) in enumerate(jobs):
  ...
  for arm in (['native','hybrid'] if index%2==0 else ['hybrid','native']):
   ...
   for num,mode in enumerate(modes):
    r=attempt(folder/f'attempt{num}',payloads[mode],source,regions,pristine,check,case,mode,host,line_protocol,structural_protocol,compact_protocol);attempts.append(r)
    if r['passed']:break
...
if __name__=='__main__':
 ...
 a=p.parse_args()
 if sum([a.line_protocol,a.structural_protocol,a.compact_protocol])>1:p.error('Choose one protocol')
 if a.semantic_targets and not a.structural_protocol:p.error('--semantic-targets requires --structural-protocol')
 main(a.out,a.host,a.repeats,a.line_protocol,a.structural_protocol,a.cases,a.semantic_targets,a.compact_protocol)
