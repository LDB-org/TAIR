...
def latest_plan_result(messages):
    ...
    calls = {}
    latest = ''
    previous_error, failure_streak = None, 0
        ...
        if message['role']=='assistant':
            ...
            if message.get('isError') and isinstance(result, dict):
                ...
                if isinstance(error, str) and error:
                    failure_streak = failure_streak+1 if error == previous_error else 1
                ...
                if failure_streak > 1:
                    latest += ('\nThe exact error output has repeated '+str(failure_streak)+' consecutive times. '
                               'Previous retries did not remove this failure, even if the command text changed. '
                               'Identify the failing prerequisite from the error and change that prerequisite before retrying. '
                               'If the cause is still unknown, inspect the relevant state instead of rerunning the same check. '
                               'This is repeated output, not proof that the underlying cause is identical.')
            else:
                previous_error, failure_streak = None, 0
...
def generic_plan_chat(payload):
    ...
    options = tool_plan.branches(tools, candidates, success_reply=success_reply,
                                 reuse_review=reuse_review, write_references=write_references, bash_argv=bash_argv)
    if recovery:
