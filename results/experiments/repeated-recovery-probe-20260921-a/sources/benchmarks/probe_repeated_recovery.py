...
def main(out,tokenizer):
    ...
    with (out/'ssh-stderr.log').open('w') as log,tempfile.TemporaryDirectory(prefix='tair-recovery-replay-') as temp:
        ...
        try:
            ...
            project=Path(temp)/'workspace';project.mkdir();b.STATE=Path(temp)/'state'
            check=Path(temp)/'unique_test.mjs'
            for cut in [3,4]:
                history=[];failures=0
                for m in messages:
                    history.append(m)
                ...
                encoded=json.dumps(history).replace(old_cwd,str(project)).replace('/private/tmp/unique_test.mjs',str(check)).replace('/tmp/unique_test.mjs',str(check)).replace('/private/tmp/unique_changed.mjs',str(Path(temp)/'unique_changed.mjs'))
