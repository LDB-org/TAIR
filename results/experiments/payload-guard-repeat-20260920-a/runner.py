...
run='payload-guard-repeat-20260920-a';out=ROOT/'results/experiments'/run;out.mkdir(exist_ok=False)
...
try:
 for number,(kind,spec) in enumerate(jobs[:1]*3,1):
  ...
  try:
   ...
   try:
    ...
    assert target.read_bytes()==(ROOT/'benchmarks/fixtures/file_io_long.py').read_bytes()
    row['correct']=True
