...
def main(args):
    ...
    for key in ('PIJIT_SCHEMA_ACTIONS', 'PIJIT_JIT_ACTIONS', 'PIJIT_LOCAL_ROUTING',
                'PIJIT_CONTINUATION_CACHE', 'PIJIT_DIRECTORY_GUARD', 'PIJIT_TOKENIZER_REVISION'):
        os.environ.pop(key, None)
    arms = ['native', 'generation', 'c4']
    (out/'manifest.json').write_text(json.dumps({'arms': arms, 'rounds': 2, 'repeats': args.repeats,
        'scenarios': SCENARIOS, 'seed': 921, 'method': 'real Pi 0.85.1; fresh sessions; restore original project before warm round; persist only project codebook; default classification reuse, no schema/preset/exact/local routing; end-to-end includes planning, tools, tests, recovery and final answer; shared remote backend over SSH'}, indent=2))
    sources = out/'sources'
    sources.mkdir()
            ...
            for name, scenario in SCENARIOS.items():
                for round_ in (1, 2):
                    ...
                    for arm in order:
                        ...
                        row['passed'] &= row['tests_unchanged']
    ...
    summary['limitations'] = 'Three small coding tasks, cold/warm pairs, real Pi loop; default learned codebook only. Different tool catalogs; no general throughput claim. Admission is compile-only; independent final behavioral oracle and unchanged tests determine success. Failed attempts and agent recovery are included.'
