...
def infer(url, messages, ids, candidates, arm, row):
    if arm == 'classify_skip_empty':
        empty = not any(candidate is not None for candidate in candidates)
        row['skipped_empty_classification'] = empty
        arm = 'generate' if empty else 'classify'
    if arm == 'generate':
...
def main(args):
    ...
    jobs=[(case,arm,repeat) for case in cases() for arm in args.arms for repeat in range(args.repeats)]
    ...
    (out/'manifest.json').write_text(json.dumps(dict(cases=cases(),label_setup_seconds=time.perf_counter()-start,
        repeats=args.repeats,arms=args.arms,scope='Synthetic Python edits, one planning/execution stage; no complete Agent.',
...
if __name__=='__main__':
    ...
    parser.add_argument('--arms', nargs='+', choices=['generate','compact','classify','classify_skip_empty'], default=['generate','compact','classify'])
