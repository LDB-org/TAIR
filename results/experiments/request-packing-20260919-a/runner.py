...
with subprocess.Popen(command,stdout=subprocess.DEVNULL,stderr=subprocess.PIPE,text=True) as tunnel:
 try:
  ...
  subprocess.run(['/tmp/tair-test-venv-20260918/bin/python','-u','benchmarks/benchmark_request_packing.py','--url',url,'--out','results/experiments/request-packing-20260919-a','--repeats','2'],env=env,check=True)
 finally:
  tunnel.terminate()
