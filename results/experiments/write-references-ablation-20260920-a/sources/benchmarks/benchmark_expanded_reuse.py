...
def main(out, continuation=False, task_ids=None, fresh=False, local_tokenizer=None, verify_tokenizer=False, prefix_cache=False, compact_catalog=False, reuse_routing=False, ssh_compression=False, ablation=False, empty_book=False, success_reply_ablation=False, recovery_latest=False, recovery_ablation=False, capabilities_ablation=False, execution_ablation=False, reuse_review_ablation=False, write_references_ablation=False):
    ...
    if success_reply_ablation or recovery_ablation or capabilities_ablation or execution_ablation or reuse_review_ablation or write_references_ablation:
        ablation = True
    cli = shutil.which('pi')
    if not cli:
    ...
    manifest['reuse_ablation'] = ablation and not success_reply_ablation and not recovery_ablation and not capabilities_ablation and not execution_ablation and not reuse_review_ablation and not write_references_ablation
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='tair-expanded-project-') as temp:
            ...
            for task_id,arm in schedule:
                ...
                overrides['PIJIT_WRITE_REFERENCES']='1' if arm=='tair_refs' else '0'
                ...
                try:
                    ...
                    if arm!='native':
                        ...
                        assert all(m.get('write_references_enabled')==(arm=='tair_refs') for m in chats), 'Runtime write reference mismatch'
