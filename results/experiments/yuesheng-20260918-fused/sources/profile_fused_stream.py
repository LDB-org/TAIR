...
class StreamingBackend(service.Backend):
    def post(self,path,payload):
        ...
        start=time.perf_counter();chunks=[];text=[];token_ids=[];usage={};finish=None;request_id=None;done=False
        ...
        return {'id':request_id,'usage':usage,'choices':[{'message':{'content':''.join(text)},'finish_reason':finish,'token_ids':token_ids}]}
