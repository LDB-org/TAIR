...
def main():
    p=argparse.ArgumentParser();p.add_argument('--url',default='http://127.0.0.1:18185');p.add_argument('--cases',required=True);p.add_argument('--out',required=True);p.add_argument('--modes',nargs='+',default=['json','hybrid_fields']);p.add_argument('--repeats',type=int,default=1)
    ...
    with (out/'rows.jsonl').open('x') as f:
        for i,c in enumerate(cases * a.repeats):
            for mode in (a.modes if i%2==0 else list(reversed(a.modes))):
                ...
                row={'repeat':i//len(cases),'id':c['id'],'mode':mode,'http_status':status,'wall_seconds':time.perf_counter()-start,'expected':expected,'exact':result.get('call')==expected,'result':result}
