...
PI = os.environ.get('PI_PACKAGE_ROOT', '/usr/local/lib/node_modules/@earendil-works/pi-coding-agent')
...
CASES = {
 ...
 'cli_limits': 'Update main CLI validation: timeout must be finite and strictly positive; workers must be an integer from 1 through 64 inclusive. Reject invalid values with argparse exit code 2. Preserve valid calls, defaults, and existing scanner behavior. No new imports.',
 'cli_defaults': 'Change CLI defaults in build_parser: --timeout defaults to 0.25 seconds and --workers defaults to 4. Keep explicit command-line overrides and all other behavior unchanged. Make minimal changes.',
}
...
elif case=='cli_limits':
 for flag,values in [('--timeout',['nan','inf','-inf','0','-1']),('--workers',['0','-1','65','100'])]:
  for value in values:
   p=subprocess.run([sys.executable,'port_scanner.py','--host','127.0.0.1','--ports','1',flag+'='+value],capture_output=True,text=True,timeout=5)
   assert p.returncode==2,(flag,value,p.returncode,p.stdout,p.stderr)
 for workers in ['1','64']:
  p=subprocess.run([sys.executable,'port_scanner.py','--host','127.0.0.1','--ports','1','--workers',workers,'--timeout','.01'],capture_output=True,text=True,timeout=5)
  assert p.returncode==0,p.stderr
elif case=='cli_defaults':
 parser=m.build_parser();args=parser.parse_args(['--host','127.0.0.1','--ports','1'])
 assert args.timeout==.25 and args.workers==4,(args.timeout,args.workers)
 args=parser.parse_args(['--host','127.0.0.1','--ports','1','--timeout','.75','--workers','3'])
 assert args.timeout==.75 and args.workers==3
...
def main(out, host, repeats, readable=False, examples=False, selected=None, multi=False, batch_only=False, modes=None):
    ...
    for repeat in range(repeats):
        for case,task in cases.items():
            region_mode='multi' if multi else 'region'
            order=modes or ['native',region_mode]
            for mode in (order if repeat%2==0 else list(reversed(order))):
                ...
                if mode=='native':
                    ...
                    if multi:
                        system+='\nYou may instead return a compact JSON array of 1 to 8 [regionID,replacement] pairs. All refer to the ORIGINAL snapshot; no duplicate or overlapping regions. Each replacement uses relative indentation, JSON escaped inside strings. Prefer multiple small regions for disjoint changes instead of rewriting unchanged code. Unrelated example: [["L8","return 4"],["L20","return 9"]]. Use the raw single-region form when it is shorter.'
                        payload['structured_outputs']={'regex':region.multi_regex(regions)}
                        if batch_only:
                            system='Return exactly a compact JSON array of 1 to 8 [regionID,replacement] pairs. Select the smallest source regions that need changing. Do not copy unchanged functions or use a large enclosing block when separate small edits suffice. All IDs refer to the ORIGINAL snapshot; duplicate or overlapping regions are forbidden. Each replacement starts at relative column zero; runtime restores base indentation. JSON-escape strings, including newlines. Empty string deletes a region. No explanations or fences.\nExample on unrelated toy source: to change return values at lines 8 and 20, output [["L8","return 4"],["L20","return 9"]].\nSOURCE HASH: '+region.digest(source)+'\nREGIONS:\n'+catalog
                            system+='\nAn L ID replaces exactly one source line, not the following block. To replace a compound statement including its body and except/finally branches, choose its listed B region. Never output original base indentation in a replacement.'
                            payload['structured_outputs']={'regex':region.multi_regex(regions,allow_single=False)}
                ...
                try:
                    ...
                    if mode=='native':
                        ...
                        if multi:
                            updated=region.decode_multi((work/'port_scanner.py').read_text(),region.digest(source),regions,raw,choice['finish_reason'])
                            result['selected_regions']=[e[0] for e in json.loads(raw)] if raw.startswith('[') else [raw.partition('\n')[0]]
                        else:
...
if __name__=='__main__':
    ...
    p.add_argument('--host',default=os.environ.get('TAIR_ENGINE_HOST', os.environ.get('ENGINE_TOOLCALL_HOST', 'localhost')));p.add_argument('--repeats',type=int,default=2)
    ...
    p.add_argument('--modes',nargs='+',choices=['native','region','multi'])
    a=p.parse_args()
    if a.multi and not a.readable_regions:p.error('--multi requires --readable-regions')
    if a.batch_only and not a.multi:p.error('--batch-only requires --multi')
    if a.modes and any(m not in ['native','multi' if a.multi else 'region'] for m in a.modes):p.error('Modes do not match protocol flags')
    main(a.out,a.host,a.repeats,a.readable_regions,a.examples,a.cases,a.multi,a.batch_only,a.modes)
