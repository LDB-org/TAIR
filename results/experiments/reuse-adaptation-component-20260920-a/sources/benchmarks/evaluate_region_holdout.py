...
spec=importlib.util.spec_from_file_location('compact',ROOT/'deploy/compact_structural_protocol.py')
compact=importlib.util.module_from_spec(spec);spec.loader.exec_module(compact)
spec=importlib.util.spec_from_file_location('applicable',ROOT/'deploy/applicable_structural_protocol.py')
applicable=importlib.util.module_from_spec(spec);spec.loader.exec_module(applicable)
CASES={
 ...
 'guard_empty_host':'In scan_port, reject an empty host string with ValueError before creating any socket. Preserve nonempty host behavior, return values, and cleanup. No new imports.',
 'json_unicode':'Make main JSON output retain Unicode host characters rather than escaping them with ASCII unicode escapes. Preserve the existing output fields, sorted keys, trailing newline and all scanner behavior.',
 'json_no_nan':'Make main JSON serialization reject NaN/Infinity in results with ValueError instead of emitting nonstandard JSON. Preserve normal JSON fields, sorted keys, trailing newline and scanner behavior.',
 'guard_port_bounds':'In scan_port, raise ValueError before creating a socket when port is outside 1..65535 inclusive. Preserve valid boundary ports, normal behavior, return values and cleanup. No new imports.',
}
...
def attempt(folder,payload,source,regions,pristine,check,case,mode,host,line_protocol=False,structural_protocol=False,compact_protocol=False,typed_protocol=False,scoped_protocol=False,applicable_protocol=False):
 ...
 try:
  ...
  if mode=='native':
   ...
   if typed_protocol:
    allowed=compact.task_scope(source,CASES[case]) if scoped_protocol else None
    decoder=applicable.decode if applicable_protocol else compact.typed_decode
    updated=decoder(source,structural.digest(source),message.get('content') or '',choice['finish_reason'],allowed)
   elif compact_protocol:
    updated=compact.decode(source,structural.digest(source),message.get('content') or '',choice['finish_reason'])
   elif structural_protocol:
...
def main(out,host,repeats,line_protocol=False,structural_protocol=False,selected=None,semantic_targets=False,compact_protocol=False,typed_protocol=False,scoped_protocol=False):
 ...
 if structural_protocol:
  ...
  templates['multi']['messages'][0]['content']=structural.prompt(source,semantic_targets)
  if semantic_targets:
   templates['multi']['messages'][1]['content']=templates['multi']['messages'][1]['content'].replace('Return one edit operation only.','Return one array containing all required edit operations.')
 if compact_protocol or typed_protocol:
  templates['multi']['structured_outputs']={'regex':compact.typed_grammar(source) if typed_protocol else compact.grammar(source)}
  templates['multi']['messages'][0]['content']=compact.typed_prompt(source) if typed_protocol else compact.prompt(source)
  numbered=''.join(f'{i}: {line}' for i,line in enumerate(source.splitlines(keepends=True),1))
  for template in templates.values():
   text=template['messages'][1]['content'].split('\nCURRENT TASK')[0]
   template['messages'][1]['content']=text.replace(numbered,source).replace('SOURCE (line numbers are not file content):','SOURCE:')
  templates['multi']['messages'][1]['content']=templates['multi']['messages'][1]['content'].replace('Return one edit operation only.','Return one array containing all required edit operations.')
  if typed_protocol:
   # Keep one task description, at the end, identically for both arms.
   for template in templates.values():
    template['messages'][1]['content']='Edit the supplied file. Make the smallest necessary change and preserve unrelated behavior.\nFILE: port_scanner.py\nSOURCE:\n'+source+'\nTASK: '+b.CASES['strict_ascii_ports']
 sources=out/'sources';sources.mkdir()
 for path in [Path(__file__),ROOT/'benchmarks/compare_region_edits.py',ROOT/'deploy/region_edit_protocol.py',ROOT/'deploy/structural_edit_protocol.py',ROOT/'deploy/compact_structural_protocol.py',ROOT/'integrations/pi/apply_edit.mjs']:
 ...
 manifest['compact_protocol']=compact_protocol
 manifest['typed_protocol']=typed_protocol
 manifest['scoped_protocol']=scoped_protocol
 boundary=len(CASES) if scoped_protocol else 12 if typed_protocol else 10
 manifest['previously_evaluated_cases']=[k for k in cases if k in list(CASES)[:boundary]]
 manifest['new_cases']=[k for k in cases if k in list(CASES)[boundary:]]
 ...
 for index,(case,rep) in enumerate(jobs):
  ...
  for mode,template in templates.items():
   ...
   if scoped_protocol and mode=='multi':
    allowed=compact.task_scope(source,CASES[case])
    payload['structured_outputs']={'regex':compact.typed_grammar(source,allowed)}
    payload['messages'][0]['content']=compact.typed_prompt(source,allowed)
  for arm in (['native','hybrid'] if index%2==0 else ['hybrid','native']):
   ...
   for num,mode in enumerate(modes):
    r=attempt(folder/f'attempt{num}',payloads[mode],source,regions,pristine,check,case,mode,host,line_protocol,structural_protocol,compact_protocol,typed_protocol,scoped_protocol);attempts.append(r)
    if r['passed']:break
...
if __name__=='__main__':
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',required=True,type=Path);p.add_argument('--host',default=os.environ.get('TAIR_ENGINE_HOST', os.environ.get('ENGINE_TOOLCALL_HOST', 'localhost')));p.add_argument('--repeats',type=int,default=2)
 ...
 p.add_argument('--semantic-targets',action='store_true')
 p.add_argument('--compact-protocol',action='store_true')
 p.add_argument('--typed-protocol',action='store_true')
 p.add_argument('--scoped-protocol',action='store_true')
 a=p.parse_args()
 if sum([a.line_protocol,a.structural_protocol,a.compact_protocol,a.typed_protocol])>1:p.error('Choose one protocol')
 if a.semantic_targets and not a.structural_protocol:p.error('--semantic-targets requires --structural-protocol')
 if a.scoped_protocol and not a.typed_protocol:p.error('--scoped-protocol requires --typed-protocol')
 main(a.out,a.host,a.repeats,a.line_protocol,a.structural_protocol,a.cases,a.semantic_targets,a.compact_protocol,a.typed_protocol,a.scoped_protocol)
