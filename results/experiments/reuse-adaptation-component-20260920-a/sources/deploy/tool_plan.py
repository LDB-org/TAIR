...
def branches(tools, candidates, success_reply=False, reuse_review=False):
    ...
    for entry in candidates:
        ...
        if reuse_review and not entry.get('plan_template'):
            ...
            reuse['properties']['arguments'] = dict(anyOf=[
                ...
                obj(dict(**review, use_cached_content=dict(const=False), path=dict(type='string'),
                         edits=dict(type='array', minItems=1, maxItems=8, items=obj(dict(
                             oldText=dict(type='string', minLength=1), newText=dict(type='string'))))))])
...
def adapt_content(source, edits):
    """Apply bounded exact replacements before any tool execution; never guess a match."""
    for edit in edits:
        old = edit['oldText']
        if not old or source.count(old) != 1:
            raise ValueError('Cached content edit requires exactly one nonempty match')
    ...
    if index < len(tools):
        ...
        for step in steps:
            ...
            if step['name'] == 'reuse_write':
                ...
                content = entry['source'] if use_cached else (
                    adapt_content(entry['source'], args['edits']) if 'edits' in args else args['content'])
                step['arguments'] = dict(path=args['path'], content=content)
                if use_cached:
                    reused.append(entry['id'])
    ...
    if len(tools) <= index < len(tools)+len(candidates):
        ...
        if reviewed:
            instruction += ('For reuse_write, FIRST write requirement_check: briefly identify the current distinguishing '
                            ...
                            'If a requirement differs, set use_cached_content=false. For a localized change prefer edits: '
                            'an ordered list of {oldText,newText} replacements against the stored text. Each nonempty oldText must '
                            'match exactly once at that point; include enough context to make it unique. Preserve all unrelated behavior. '
                            'For broad changes provide corrected full content instead, or use ordinary tool steps. '
                            'A changed destination alone is not a conflict. ')
