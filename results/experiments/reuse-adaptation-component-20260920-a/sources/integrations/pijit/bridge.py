...
def generic_plan_chat(payload):
    ...
    adapted_steps = sum(step['name']=='reuse_write' and 'edits' in step['arguments']
                        for step in response['call']['arguments'].get('steps', []))
    trace_update(adapted_content_steps=adapted_steps)
    cache_outcome = ('reply' if call['name']=='reply_user' else 'reused' if reused else
                     'adapted_content' if adapted_steps else
...
def final_written_contents(payload):
    cwd = Path(payload['cwd']).resolve()
    contents, skipped = {}, []
    for name in dict.fromkeys(s['arguments']['path'] for s in payload['steps'] if s['name']=='write'):
        try:
...
def complete_tool_plan(payload):
    ...
    book = tool_plan.ToolContentBook(paths(payload['cwd'])/'tool-plan-codebook.sqlite3')
    final_contents = final_written_contents(payload)
    ...
    writes = list(dict.fromkeys(content for content in final_contents.values() if content not in reused_sources))
    ...
    return dict(admitted=admitted, cache_hit=bool(reused or reused_templates), admission_count=len(admitted),
                ...
                admission_reason='new_write_content' if admitted else 'deduplicated_or_reused' if final_contents else
                'no_eligible_final_content' if any(s['name']=='write' for s in payload['steps']) else 'no_write_steps',
