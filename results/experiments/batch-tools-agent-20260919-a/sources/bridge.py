...
def chat(payload):
    ...
    if plan:
        system += ('\nPrefer execute_plan to perform 2 to 8 already-determined operations in one turn. '
...
def batch_tool(tools):
    ...
    return {'name': 'execute_plan', 'description':
            'Execute 2 to 8 known tool calls sequentially, including edits followed by tests; stop on error.',
            'parameters': {'type': 'object', 'properties': {'steps': {
                'type': 'array', 'minItems': 2, 'maxItems': 8, 'items': {'oneOf': steps}}},
