...
def main(args):
    ...
    for key in ('PIJIT_SCHEMA_ACTIONS', 'PIJIT_JIT_ACTIONS', 'PIJIT_LOCAL_ROUTING',
                'PIJIT_CONTINUATION_CACHE', 'PIJIT_DIRECTORY_GUARD', 'PIJIT_TOKENIZER_REVISION', 'PIJIT_BATCH_TOOLS'):
                ...
                for round_ in (1, 2):
                    ...
                    for arm in order:
                        ...
                        for key in ('PIJIT_LOCAL_ROUTING', 'PIJIT_CONTINUATION_CACHE', 'PIJIT_DIRECTORY_GUARD'):
                            os.environ[key] = str(int(arm in ('optimized', 'batched')))
                        os.environ['PIJIT_BATCH_TOOLS'] = str(int(arm == 'batched'))
                        if arm in ('optimized', 'batched'):
...
if __name__ == '__main__':
    ...
    if (args.outer_optimized or args.batch_comparison) and not args.tokenizer_revision:
