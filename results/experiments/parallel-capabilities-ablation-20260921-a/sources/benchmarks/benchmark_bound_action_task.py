...
bridge = load('bound_bridge', ROOT / 'integrations/pijit/bridge.py')
...
def bound(project, folder, value, arm, rng):
    ...
    classification = arm in ('classify', 'classify_cached')
    try:
        ...
        instruction = ('Return only the letter of the complete action that satisfies the task. Select null if none applies.'
                       if classification else 'Return only the complete action JSON that satisfies the task, or null if none applies.')
        ...
        if arm == 'classify_cached':
            trace = {'http_requests': [], 'stage_seconds': {}}
            token = bridge.TRACE.set(trace)
            try:
                tokens, ids = bridge.prepare_classification(messages, len(labels))
            finally:
                bridge.TRACE.reset(token)
            row['preparation_trace'] = trace
            row['label_cache_hit'] = trace.get('label_cache_hit', False)
        else:
        ...
        row.update(http_seconds=time.perf_counter() - begin, response=response,
                   generated_tokens=0 if classification else response['usage']['completion_tokens'],
                   control_records=int(classification))
...
def main(args):
    ...
    (out / 'sources').mkdir()
    selected_arms = ['native', 'generate', 'classify'] + (['classify_cached'] if args.optimized else [])
    if args.optimized and not os.environ.get('PIJIT_TOKENIZER_REVISION'):
        raise ValueError('Optimized comparison requires verified PIJIT_TOKENIZER_REVISION')
    for f in [Path(__file__), ROOT/'benchmarks/compare_pijit_presets.py', ROOT/'benchmarks/native_pi_trace.ts',
              ROOT/'benchmarks/benchmark_ideal_classification.py', ROOT/'deploy/preset_edits.py', ROOT/'integrations/pijit/bridge.py']:
    ...
    (out/'manifest.json').write_text(json.dumps({'scope': 'Bounded argparse --workers integer default fixture. Six prebound candidates including NONE, values fixed independently of expected answer. Not a general Agent.',
        'arms': selected_arms, 'tokenizer_revision': os.environ.get('PIJIT_TOKENIZER_REVISION'),
        'optimized': 'Existing revision-bound label cache plus parallel label/message preparation. Fresh cache per repeat: first cached-arm task cold, second warm; all times included. Cache files remain temporary.', 'repeats': args.repeats, 'values': [6,8], 'seed': 20260919,
        'timing': 'Bound arms include source read, binding, prompt tokenization and any label cache lookup/fill, model HTTP, local execution and independent behavior validation. Native includes Pi startup/read/edit/reply and same final oracle. Bound arms use deterministic DONE and one model call; native remains general Pi loop. Bound Python interpreter/import startup excluded. No retries or warm-up; all samples retained.',
        'causality': 'generate versus classify isolates output mechanism with same specialized workflow; native comparison also changes orchestration. The classify arm keeps six sequential label tokenizer calls; classify_cached reuses the revision-bound bridge cache and overlaps cold preparation.',
        'server': 'Existing shared patched vLLM, max-num-seqs 4; no restart/configuration change.'},indent=2))
    cache_root = tempfile.TemporaryDirectory(prefix='tair-bound-label-cache-')
    rng=random.Random(20260919)
    for repeat in range(args.repeats):
        bridge.STATE = Path(cache_root.name) / str(repeat)
        for value in [6,8]:
            arms=list(selected_arms);rng.shuffle(arms)
