            ...
            for block in message.get('content', []):
                if isinstance(block, dict) and block.get('type') == 'toolCall':
                    ...
                    if block['name'] == 'compact_edit':
                        ...
                        attempted.add((str((cwd / args['path']).resolve()), args['task'].strip()))
    ...
    for task in tasks:
        ...
        for path in sorted(paths_read):
            ...
            try:
                ...
                continue
        if len(candidates) == 1:
            call = {'name': 'compact_edit', 'arguments': {'path': str(candidates[0].relative_to(cwd)), 'task': task}}
...
def chat(payload):
    if os.environ.get('PIJIT_LOCAL_ROUTING') == '1':
    ...
    if os.environ.get('PIJIT_SCHEMA_ACTIONS') == '1' or os.environ.get('PIJIT_JIT_ACTIONS') == '1':
        system += ('\nUse compact_edit for existing Python call keyword changes (including CLI integer/float/string '
                   'defaults, help and required), option aliases, entry guards, and catch/return. '
                   'Use one atomic change per call. Pass the requested edit clause verbatim as task when possible; '
                   'do not add test or summary instructions to that clause. For CLI properties use the precise form '
                   'Change --option property to VALUE. Quote string values. Preserve normal edit/write for '
                   'unsupported transformations and new files. Read files, not directories; use bash to list directories.')
...
def pick_cached(source, task, options):
    ...
    with stage('cache_preparation'):
        ...
        prompt, ids = prepare_classification([{'role': 'system', 'content': 'Select a cached edit ONLY if it satisfies the entire task. Otherwise NONE. Return its letter.\n' + choices},
                           {'role': 'user', 'content': 'SOURCE:\n' + source + '\nTASK:\n' + task}], len(candidates))
    ...
    bound = bound_keyword_edit(source, task)
    ...
    if not bound_match and gate['conditional_probability'] < 0.95:
        reasons.append('low_conditional_probability')
    if not bound_match and gate['margin'] < 3:
        reasons.append('low_margin')
    accepted = not reasons
    decision.update(gate=gate, accepted=accepted, rejection_reasons=reasons,
                    acceptance_basis=('explicit_binding' if bound_match else 'score_gate') if accepted else None)
