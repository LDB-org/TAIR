...
p=root/'results/experiments/bound-plan-20260919-a'
rows=[json.loads(s) for s in (p/'rows.jsonl').read_text().splitlines()]
assert len(rows)==45 and len({(r['arm'],r['case'],r['repeat']) for r in rows})==45
...
ratios={scope:dict(classify_vs_full_saved=1-index[scope,'classify']['total_seconds']/index[scope,'full']['total_seconds'],
