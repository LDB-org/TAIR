...
class Protocol:
    def __init__(self, tools, raw_required_tail=False):
...
def infer(request, upstream):
    ...
    if request.get('protocol_mode', 'engine') not in ('engine', 'engine_raw'):
        raise ValueError('Unsupported protocol mode')
    protocol = Protocol(request['tools'], raw_required_tail=request.get('protocol_mode') == 'engine_raw')
