...
def main(args):
    ...
    (out / 'manifest.json').write_text(json.dumps(dict(prompt=PROMPT, arms=arms, configurations=configs,
        source_sha256=hashes, task_timeout=args.timeout, repetitions=1, empty_codebooks=True,
        method='Native first, TAIR second, one attempt each. Same absolute workspace reset to empty scanner.py, separate fresh state. Same backend and generation cap. Hidden loopback oracle; no runtime verification command. No retries. Timing includes Agent and separately recorded oracle; excludes fixture/tunnel setup.'), indent=2))
    scenario = dict(primary_file='scanner.py', files={'scanner.py': ''}, prompt=PROMPT, check=ORACLE)
    rows = []
    with tempfile.TemporaryDirectory(prefix='tair-port-scanner-') as tmp:
        project = Path(tmp) / 'project'
        for arm in arms:
            if project.exists():shutil.rmtree(project)
            project.mkdir(); (project / 'scanner.py').write_text('')
