...
def generic_plan_chat(payload):
    ...
    response = infer(messages, [dict(option, parameters=tool_plan.decoder_schema(option['parameters'])) for option in options],
                     branch_instruction=lambda index, option: tool_plan.continuation(
...
def reject_repeated_plan(call, messages):
    if call['name'] != 'plan' or not any(s['name'] in ('write','edit') for s in call['arguments']['steps']):
        return
    latest = max((i for i, m in enumerate(messages) if m['role']=='user'), default=-1)
    previous, last_completed = {}, None
    for message in messages[latest+1:]:
        if message['role']=='assistant':
            ...
            last_completed = None if message.get('isError') else previous.get(message.get('toolCallId'))
    if last_completed == call['arguments']:
