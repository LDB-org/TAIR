...
run='production-ten-20260920-a';out=ROOT/'results/experiments'/run;out.mkdir(exist_ok=False)
...
try:
 for number,(kind,spec) in enumerate(jobs*2,1):
  ...
  offset=len(records());start=time.perf_counter()
  try:
   ...
   row=dict(task=number,kind=kind,repeat=number>5,seconds=elapsed,returncode=done.returncode,records=current)
  ...
  rows.append(row);(out/'report.json').write_text(json.dumps(rows,indent=2));print(json.dumps({k:v for k,v in row.items() if k!='records'}),flush=True)
finally:
