...
from typing import Iterable, List, Optional, Sequence, Set, Tuple
    ...
    Accepts individual ports, inclusive ranges (e.g. 80,443,8000-8010), and
    inclusive stepped ranges (e.g. 20-26/2 gives 20,22,24,26).
    ...
    for raw_part in spec.split(","):
        ...
        if "-" in part:
            ...
            step = 1
            if "/" in right:
                right, step_str = right.split("/", 1)
                right = right.strip()
                step_str = step_str.strip()
                if not step_str:
                    raise ValueError(f"malformed port range: {part!r}")
                try:
                    step = int(step_str, 10)
                except ValueError:
