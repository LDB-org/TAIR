...
def main(out,tokenizer,pi_context=False):
    out=out.resolve()
    ...
    manifest=dict(pi_context=pi_context,fixtures=fixtures,guidance=GUIDANCE,source_sha256=sources,method='One natural classification and generated plan per arm and fixture; identical empty-book context and schemas; no tool execution, Agent completion, or end-to-end speed claim.')
...
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',type=Path,required=True);p.add_argument('--tokenizer',type=Path,required=True);p.add_argument('--pi-context',action='store_true');a=p.parse_args();main(a.out,a.tokenizer,a.pi_context)
