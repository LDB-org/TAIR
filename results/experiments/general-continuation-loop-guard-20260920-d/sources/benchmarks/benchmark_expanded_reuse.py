...
def main(out, continuation=False, task_ids=None):
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='tair-expanded-project-') as temp:
            ...
            tunnel=subprocess.Popen(['ssh','-N','-o','BatchMode=yes','-o','StrictHostKeyChecking=yes','-o','ExitOnForwardFailure=yes','-L',f'127.0.0.1:{port}:127.0.0.1:8000','rs-yuesheng-gpu-public'],stdout=subprocess.DEVNULL,stderr=subprocess.PIPE)
...
if __name__=='__main__':
    ...
    args=p.parse_args();main(args.out,args.continuation,args.tasks)
