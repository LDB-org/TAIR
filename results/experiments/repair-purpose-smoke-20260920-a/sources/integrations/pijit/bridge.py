    ...
    for message in messages[latest_user+1:]:
        if message['role']=='assistant':
            ...
            if message.get('isError'):
                failure = latest
    if failure and latest != failure:
        return failure+'\n'+latest+'\nA successful read or diagnostic does not by itself resolve the earlier failure; verify the corrective action.'
    return latest
...
def generic_plan_chat(payload):
    ...
    candidates = book.candidates(task, {}, limit=min(7, 15-len(tools))) if any(t['name']=='write' for t in tools) else []
    template_book = None
    if os.environ.get('PIJIT_PLAN_TEMPLATES') == '1':
        ...
        templates = [dict(entry, plan_template=True) for entry in template_book.candidates(task, {}, limit=7)]
        candidates = (templates + candidates)[:min(7, 15-len(tools))]
    before_payload_filter = len(candidates)
    candidates = [dict(entry, request_delta=tool_plan.request_delta(entry['contract'], task))
                  for entry in candidates if tool_plan.compatible_payload(entry, task)]
    trace_update(payload_mismatch_candidates=before_payload_filter-len(candidates))
    ...
    options = tool_plan.branches(tools, candidates)
    latest_user = max((i for i, m in enumerate(context['messages']) if m['role']=='user'), default=-1)
    recovery = any(m['role']=='toolResult' and m.get('isError') for m in context['messages'][latest_user+1:])
    if recovery:
        options = [tool_plan.recovery_option(options[-1])]
    trace_update(recovery_planning=recovery)
    policy = (context.get('systemPrompt', '') +
              ...
              '\nINNER TOOLS: '+json.dumps(tools, ensure_ascii=False))
    ...
    if recovery:
        execution_state += '\nCURRENT USER REQUIREMENTS:\n'+task+'\nIf a concrete fix is known, execute it now; explaining a future fix is not completion. '
    ...
    response = infer(messages, [dict(option, parameters=tool_plan.decoder_schema(option['parameters'])) for option in options],
                     branch_instruction=lambda index, option: execution_state+'\n'+tool_plan.continuation(
                         len(tools)+len(candidates) if recovery else index, tools, candidates, option),
                     plan_budget=budget_enabled,
                     classification_prompt=('A previous tool failed in this user task. Choose A for general replanning. '
                     ...
                     'For pending writes, compare the candidate HISTORICAL CONTRACT and SOURCE previews with the CURRENT request. '
                     'Select a matching entry to inspect its full contract and source in continuation, then reuse or generate. '
    ...
    call, reused = tool_plan.decode(response, tools, candidates, options)
    ...
    return dict(response, call=call, plan_task=task, reused_content_ids=reused_contents,
