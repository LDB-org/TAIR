...
def continuation(index, tools, candidates, option, success_reply=False):
    ...
    if index < len(tools):
        ...
        if any('steps' in variant.get('properties', {}) for variant in option['parameters'].get('anyOf', [])):
            instruction += ('If the selected first action conflicts with the current request or required operation order, '
                            'correct it now by returning steps with the appropriate native tools instead of first/rest. '
                            'Do not execute an unnecessary selected action before the required first operation. ')
