...
def main(out,tokenizer,candidate_bridge):
    out.mkdir(parents=True,exist_ok=False)
    b=load('recovery_bridge',candidate_bridge)
    ...
    (out/'candidate_bridge.py').write_bytes(candidate_bridge.read_bytes())
    ...
    (out/'manifest.json').write_text(json.dumps(dict(source_experiment=prior.name,cuts=[3,4],
        ...
        candidate_bridge=str(candidate_bridge),candidate_sha256=hashlib.sha256(candidate_bridge.read_bytes()).hexdigest(),
...
if __name__=='__main__':
    ...
    a=p.parse_args();main(a.out.resolve(),a.tokenizer.resolve(),a.candidate_bridge.resolve())
