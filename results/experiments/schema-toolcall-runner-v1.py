...
SYSTEM = (
    "Produce one tool call for the user's request. Never execute a tool. "
...
def run(model, tokenizer, case, mode, limit):
    ...
    with torch.inference_mode():
        if mode == "json":
            messages[-1]["content"] += "\nReturn the complete tool call as compact JSON only."
            ...
            messages.append({"role": "user", "content": f"Selected tool: {name}. Select arguments.priority."})
            priority = session.choose(messages, ["normal", "urgent"])
            messages.append({"role": "user", "content": (
                f"Selected tool: {name}; priority: {priority}. "
                "Output only the raw text value of arguments.content. Do not use JSON, wrapping quotes, or a code fence."
            )})
            raw, tokens, complete = session.generate(messages, limit)
    ...
    return {
        ...
        "generated_tokens_including_eos": tokens, "classification_decisions": 2 if mode == "hybrid" else 0,
...
def main():
    ...
    report = {
        "schema": "openjev-schema-toolcall-toy-v1", "model": metadata,
        ...
        "limits": ["Six authored routing/copy cases, not an agent benchmark", "No tools executed",
