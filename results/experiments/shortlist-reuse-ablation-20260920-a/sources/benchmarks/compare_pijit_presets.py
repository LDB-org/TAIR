...
def attempt(project, state, folder, arm, value, timeout, scenario=None, env_overrides=None, append_system_prompt=None):
    ...
    primary = project / (scenario.get('primary_file', 'app.py') if scenario else 'app.py')
    (folder / 'before.py').write_bytes(primary.read_bytes())
    ...
    with tempfile.TemporaryDirectory(prefix='pijit-benchmark-profile-') as profile, \
        ...
        try:
            if arm == 'native':
                if env.get('TAIR_NATIVE_PREFIX_CACHE') == '1':
                    env['TAIR_NATIVE_CACHE_SALT'] = hashlib.sha256(str(state.resolve()).encode()).hexdigest()
                ...
                command = ['node', cli, '--no-extensions', '-e', str(ROOT / 'benchmarks/native_pi_trace.ts'),
                           ...
                           env.get('TAIR_NATIVE_TOOLS', 'read,edit,write,bash' if scenario is not None else 'read,edit'), '-p', prompt]
    ...
    try:
        ...
        validation = {'passed': False, 'seconds': time.perf_counter() - checked_at, 'stderr': 'Independent oracle timed out after 15 seconds'}
    (folder / 'after.py').write_bytes(primary.read_bytes() if primary.is_file() else b'')
