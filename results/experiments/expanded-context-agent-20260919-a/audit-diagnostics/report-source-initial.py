...
def report(out):
    ...
    for filename in ['requested-test-review.json','cli-test-review.json']:
        if (out/filename).exists():
            rejected_tests.update(r['case'] for r in json.loads((out/filename).read_text()) if not r['requested_tests_effective'])
    rows = [{**r, 'base_passed': r['passed'], 'requested_tests_passed': (f"{r['repeat']}-{r['case']}-{r['arm']}-{r['round']}" not in rejected_tests),
    ...
    for r in failed:lines.append(f"- {r['repeat']}-{r['case']}-{r['arm']}-{r['round']}: oracle={r['validation']['passed']}, test integrity={r['test_integrity']}, requested-test coverage={r['requested_tests_passed']}, timeout={r['timed_out']}, complete usage={r['usage_complete']}.")
