  ...
  offer(id, text, history, steps, results, session, expectedOutputs = []) {
    ...
    for (const output of expectedOutputs) {
      if (!Number.isInteger(output.step) || steps[output.step]?.name !== 'bash'
          || typeof output.text !== 'string' || expected.has(output.step)) return false;
    ...
    const safe = steps.every((step, index) => {
      ...
      return step.name === 'bash' && !entry.result.details?.truncated
        && !entry.result.details?.truncation?.truncated
        && !entry.result.details?.fullOutputPath
        && entry.result.content.every(block => block.type === 'text')
        && (expected.has(index)
          ? entry.result.content.map(block => block.text).join('') === expected.get(index)
          : entry.result.content.every(block => ['', '(no output)'].includes(block.text.trim())));
    });
