...
const cliArgs = process.argv.slice(2).filter(arg => !['--plan-only', '--verified-modules'].includes(arg));
const planOnly = process.argv.includes('--plan-only') || process.env.PIJIT_PLAN_ONLY === '1';
const verifiedModules = process.argv.includes('--verified-modules');
if (planOnly && verifiedModules) {
  let validator;
...
try {
  ...
  const env = { ...process.env, PIJIT_STATE_DIR: state, PI_CODING_AGENT_DIR: join(state, 'agent'),
    ...
    ...(planOnly ? { PIJIT_PLAN_ONLY: '1', PIJIT_TOOL_PLAN: verifiedModules ? '0' : '1',
      PIJIT_ADAPTIVE_PLAN: verifiedModules ? '1' : '0', PIJIT_NATIVE_PLANNER: '1', PIJIT_PLAN_DISABLE_REUSE: process.env.PIJIT_PLAN_DISABLE_REUSE ?? '0' } : {}) };
