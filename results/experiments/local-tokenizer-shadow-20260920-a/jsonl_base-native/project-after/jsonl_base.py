...
Each non-blank line is parsed independently with the standard library's
``json.loads``. Blank lines (whitespace-only) are skipped. Lines are
...
def parse_jsonl(text):
    ...
    Duplicate object keys follow standard ``json.loads`` behavior:
    the last value wins.
    """
    values = []
    for line in text.splitlines():
        ...
        try:
            ...
            raise ValueError(
                "Malformed JSON line: {!r}".format(line)
            ) from exc
    return values
