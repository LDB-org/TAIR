...
def main(out):
    ...
    with tempfile.TemporaryDirectory(prefix='tair-generic-plan-') as temp:
        ...
        jobs=[('cold','Create constants.mjs containing exactly "export const answer = 42;" followed by one newline. Then run node --check constants.mjs via bash. Do not create any other file.'),
              ...
              ('search','Use ls, find with pattern *.mjs, and grep with pattern workers to inspect the workspace. Report matching filenames. Do not modify or create files.')]
        rows=[];previous=0
        try:
            for name,prompt in jobs:
                ...
                if name in ('cold','warm'):
                    ...
                    row['reuse_hit']=any(r.get('reused_content_ids') for r in chats)
                    ...
                    assert {'ls','find','grep'} <= set(actions)
                    assert not {'write','edit','bash'} & set(actions)
            ...
            for path in (root/'state').glob('workspaces/*/plan-events.jsonl'):shutil.copyfile(path,out/path.name)
