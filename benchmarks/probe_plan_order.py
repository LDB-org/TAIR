...
def main(out,tokenizer,pi_context=False,forced_fallback=False,bridge_path=None):
    if forced_fallback and bridge_path is None:raise ValueError("Forced fallback requires an explicit frozen candidate --bridge")
    out=out.resolve()
    ...
    bridge_path=bridge_path or ROOT/'integrations/pijit/bridge.py'
    b=load('order_bridge',bridge_path)
    ...
    manifest=dict(candidate_bridge=str(bridge_path.resolve()),candidate_bridge_sha256=hashlib.sha256(bridge_path.read_bytes()).hexdigest(),forced_fallback=forced_fallback,pi_context=pi_context,fixtures=fixtures,guidance=GUIDANCE,source_sha256=sources,method=('Force the wrong native branch using a singleton allowed label, then generate once; identical empty-book context but native fallback schema and continuation differ; no tool execution or Agent speed claim.' if forced_fallback else 'One natural classification and generated plan per arm and fixture; identical empty-book context and schemas; no tool execution, Agent completion, or end-to-end speed claim.'))
...
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--out',type=Path,required=True);p.add_argument('--tokenizer',type=Path,required=True);p.add_argument('--pi-context',action='store_true');p.add_argument('--forced-fallback',action='store_true');p.add_argument('--bridge',type=Path);a=p.parse_args();main(a.out,a.tokenizer,a.pi_context,a.forced_fallback,a.bridge)
